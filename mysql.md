...
- [1. MySQL](#1-mysql)
    ...
    - [1.30. SQL实战](#130-sql实战)
        - [1.30.1. 查找最晚入职员工的所有信息](#1301-查找最晚入职员工的所有信息)
        - [1.30.2. 查找入职员工时间排名倒数第三的员工所有信息](#1302-查找入职员工时间排名倒数第三的员工所有信息)
        - [1.30.3. 查找各个部门当前(to_date='9999-01-01')领导当前薪水详情以及其对应部门编号dept_no](#1303-查找各个部门当前to_date9999-01-01领导当前薪水详情以及其对应部门编号dept_no)
        - [1.30.4. 查找所有已经分配部门的员工的last_name和first_name](#1304-查找所有已经分配部门的员工的last_name和first_name)
        - [1.30.5. 查找所有员工的last_name和first_name以及对应部门编号dept_no，也包括展示没有分配具体部门的员工](#1305-查找所有员工的last_name和first_name以及对应部门编号dept_no也包括展示没有分配具体部门的员工)
        - [1.30.6. 查找所有员工入职时候的薪水情况，给出emp_no以及salary， 并按照emp_no进行逆序](#1306-查找所有员工入职时候的薪水情况给出emp_no以及salary-并按照emp_no进行逆序)
        - [1.30.7. 查找薪水涨幅超过15次的员工号emp_no以及其对应的涨幅次数t](#1307-查找薪水涨幅超过15次的员工号emp_no以及其对应的涨幅次数t)
        - [1.30.8. 找出所有员工当前(to_date='9999-01-01')具体的薪水salary情况，对于相同的薪水只显示一次,并按照逆序显示](#1308-找出所有员工当前to_date9999-01-01具体的薪水salary情况对于相同的薪水只显示一次并按照逆序显示)
        - [1.30.9. 获取所有部门当前manager的当前薪水情况，给出dept_no, emp_no以及salary，当前表示to_date='9999-01-01'](#1309-获取所有部门当前manager的当前薪水情况给出dept_no-emp_no以及salary当前表示to_date9999-01-01)
        - [1.30.10. 获取所有非manager的员工emp_no](#13010-获取所有非manager的员工emp_no)
        - [1.30.11. 获取所有员工当前的manager，如果当前的manager是自己的话结果不显示，当前表示to_date='9999-01-01'。](#13011-获取所有员工当前的manager如果当前的manager是自己的话结果不显示当前表示to_date9999-01-01)
        - [1.30.12. 获取所有部门中当前员工薪水最高的相关信息，给出dept_no, emp_no以及其对应的salary](#13012-获取所有部门中当前员工薪水最高的相关信息给出dept_no-emp_no以及其对应的salary)
        - [1.30.13. 从titles表获取按照title进行分组，每组个数大于等于2，给出title以及对应的数目t。](#13013-从titles表获取按照title进行分组每组个数大于等于2给出title以及对应的数目t)
        - [1.30.14. 从titles表获取按照title进行分组，每组个数大于等于2，给出title以及对应的数目t。](#13014-从titles表获取按照title进行分组每组个数大于等于2给出title以及对应的数目t)
        - [1.30.15. 查找employees表所有emp_no为奇数，且last_name不为Mary的员工信息，并按照hire_date逆序排列](#13015-查找employees表所有emp_no为奇数且last_name不为mary的员工信息并按照hire_date逆序排列)
        - [1.30.16. 统计出当前各个title类型对应的员工当前薪水对应的平均工资。结果给出title以及平均工资avg。](#13016-统计出当前各个title类型对应的员工当前薪水对应的平均工资结果给出title以及平均工资avg)
        - [1.30.17. 获取当前（to_date='9999-01-01'）薪水第二多的员工的emp_no以及其对应的薪水salary](#13017-获取当前to_date9999-01-01薪水第二多的员工的emp_no以及其对应的薪水salary)
        - [1.30.18. 查找当前薪水(to_date='9999-01-01')排名第二多的员工编号emp_no、薪水salary、last_name以及first_name，不准使用order by](#13018-查找当前薪水to_date9999-01-01排名第二多的员工编号emp_no薪水salarylast_name以及first_name不准使用order-by)
        - [1.30.19. 查找所有员工的last_name和first_name以及对应的dept_name，也包括暂时没有分配部门的员工](#13019-查找所有员工的last_name和first_name以及对应的dept_name也包括暂时没有分配部门的员工)
        - [1.30.20. 查找员工编号emp_no为10001其自入职以来的薪水salary涨幅值growth](#13020-查找员工编号emp_no为10001其自入职以来的薪水salary涨幅值growth)
        - [1.30.21. **查找所有员工自入职以来的薪水涨幅情况，给出员工编号emp_no以及其对应的薪水涨幅growth，并按照growth进行升序](#13021-查找所有员工自入职以来的薪水涨幅情况给出员工编号emp_no以及其对应的薪水涨幅growth并按照growth进行升序)
        - [1.30.22. 统计各个部门对应员工涨幅的次数总和，给出部门编码dept_no、部门名称dept_name以及次数sum](#13022-统计各个部门对应员工涨幅的次数总和给出部门编码dept_no部门名称dept_name以及次数sum)
        - [1.30.23. 对所有员工的当前(to_date='9999-01-01')薪水按照salary进行按照1-N的排名，相同salary并列且按照emp_no升序排列](#13023-对所有员工的当前to_date9999-01-01薪水按照salary进行按照1-n的排名相同salary并列且按照emp_no升序排列)
        - [1.30.24. 获取所有非manager员工当前的薪水情况，给出dept_no、emp_no以及salary ，当前表示to_date='9999-01-01'](#13024-获取所有非manager员工当前的薪水情况给出dept_noemp_no以及salary-当前表示to_date9999-01-01)
        - [1.30.25. 获取员工其当前的薪水比其manager当前薪水还高的相关信息，当前表示to_date='9999-01-01',](#13025-获取员工其当前的薪水比其manager当前薪水还高的相关信息当前表示to_date9999-01-01)
        - [1.30.26. 汇总各个部门当前员工的title类型的分配数目，结果给出部门编号dept_no、dept_name、其当前员工所有的title以及该类型title对应的数目count](#13026-汇总各个部门当前员工的title类型的分配数目结果给出部门编号dept_nodept_name其当前员工所有的title以及该类型title对应的数目count)
        - [1.30.27. 给出每个员工每年薪水涨幅超过5000的员工编号emp_no、薪水变更开始日期from_date以及薪水涨幅值salary_growth，并按照salary_growth逆序排列。](#13027-给出每个员工每年薪水涨幅超过5000的员工编号emp_no薪水变更开始日期from_date以及薪水涨幅值salary_growth并按照salary_growth逆序排列)
        - [1.30.28. 查找描述信息中包括robot的电影对应的分类名称以及电影数目，而且还需要该分类对应电影数量>=5部](#13028-查找描述信息中包括robot的电影对应的分类名称以及电影数目而且还需要该分类对应电影数量5部)
        - [1.30.29. 使用join查询方式找出没有分类的电影id以及名称](#13029-使用join查询方式找出没有分类的电影id以及名称)
        - [1.30.30. 使用子查询的方式找出属于Action分类的所有电影对应的title,description](#13030-使用子查询的方式找出属于action分类的所有电影对应的titledescription)
        - [1.30.31. 获取select](#13031-获取select)
        - [1.30.32. 将employees表的所有员工的last_name和first_name拼接起来作为Name，中间以一个空格区分](#13032-将employees表的所有员工的last_name和first_name拼接起来作为name中间以一个空格区分)
        - [1.30.33. 创建一个actor表，包含如下列信息](#13033-创建一个actor表包含如下列信息)
        - [1.30.34. 批量插入数据](#13034-批量插入数据)
        - [1.30.35. 批量插入数据，不使用replace操作](#13035-批量插入数据不使用replace操作)
        - [1.30.36. 创建一个actor_name表](#13036-创建一个actor_name表)
        - [1.30.37. 对first_name创建唯一索引uniq_idx_firstname](#13037-对first_name创建唯一索引uniq_idx_firstname)
        - [1.30.38. 针对actor表创建视图actor_name_view](#13038-针对actor表创建视图actor_name_view)
        - [1.30.39. 针对上面的salaries表emp_no字段创建索引idx_emp_no](#13039-针对上面的salaries表emp_no字段创建索引idx_emp_no)
        - [1.30.40. 在last_update后面新增加一列名字为create_date](#13040-在last_update后面新增加一列名字为create_date)
        - [1.30.41. 构造一个触发器audit_log](#13041-构造一个触发器audit_log)
        - [1.30.42. 删除emp_no重复的记录，只保留最小的id对应的记录。](#13042-删除emp_no重复的记录只保留最小的id对应的记录)
        - [1.30.43. 将所有to_date为9999-01-01的全部更新为NULL](#13043-将所有to_date为9999-01-01的全部更新为null)
        - [1.30.44. 将id=5以及emp_no=10001的行数据替换成id=5以及emp_no=10005](#13044-将id5以及emp_no10001的行数据替换成id5以及emp_no10005)
        - [1.30.45. 将titles_test表名修改为titles_2017](#13045-将titles_test表名修改为titles_2017)
        - [1.30.46. 在audit表上创建外键约束，其emp_no对应employees_test表的主键id](#13046-在audit表上创建外键约束其emp_no对应employees_test表的主键id)
        - [1.30.47. 如何获取emp_v和employees有相同的数据no](#13047-如何获取emp_v和employees有相同的数据no)
        - [1.30.48. 将所有获取奖金的员工当前的薪水增加百分之十](#13048-将所有获取奖金的员工当前的薪水增加百分之十)
        - [1.30.49. 针对库中的所有表生成select count(*)对应的SQL语句](#13049-针对库中的所有表生成select-count对应的sql语句)
        - [1.30.50. 将employees表中的所有员工的last_name和first_name通过(')连接起来。](#13050-将employees表中的所有员工的last_name和first_name通过连接起来)
        - [1.30.51. 查找字符串'10,A,B' 中逗号','出现的次数cnt](#13051-查找字符串10ab-中逗号出现的次数cnt)
        - [1.30.52. 获取Employees中的first_name](#13052-获取employees中的first_name)
        - [1.30.53. 按照dept_no进行汇总](#13053-按照dept_no进行汇总)
        - [1.30.54. 查找排除当前最大、最小salary之后的员工的平均工资avg_salary](#13054-查找排除当前最大最小salary之后的员工的平均工资avg_salary)
        - [1.30.55. 分页查询employees表，每5行一页，返回第2页的数据](#13055-分页查询employees表每5行一页返回第2页的数据)
        - [1.30.56. 获取所有员工的emp_no](#13056-获取所有员工的emp_no)
        - [1.30.57. 使用含有关键字exists查找未分配具体部门的员工的所有信息。](#13057-使用含有关键字exists查找未分配具体部门的员工的所有信息)
        - [1.30.58. 获取employees中的行数据，且这些行也存在于emp_v中](#13058-获取employees中的行数据且这些行也存在于emp_v中)
        - [1.30.59. 获取有奖金的员工相关信息。](#13059-获取有奖金的员工相关信息)
        - [1.30.60. 统计salary的累计和running_total](#13060-统计salary的累计和running_total)
        - [1.30.61. 对于employees表中，给出奇数行的first_name](#13061-对于employees表中给出奇数行的first_name)
...
### 1.30.3. 查找各个部门当前(to_date='9999-01-01')领导当前薪水详情以及其对应部门编号dept_no
...
### 1.30.4. 查找所有已经分配部门的员工的last_name和first_name
<a href="#menu" style="float:right">目录</a> 
...
### 1.30.5. 查找所有员工的last_name和first_name以及对应部门编号dept_no，也包括展示没有分配具体部门的员工
<a href="#menu" style="float:right">目录</a> 
...
### 1.30.6. 查找所有员工入职时候的薪水情况，给出emp_no以及salary， 并按照emp_no进行逆序
<a href="#menu" style="float:right">目录</a> 
...
### 1.30.7. 查找薪水涨幅超过15次的员工号emp_no以及其对应的涨幅次数t
<a href="#menu" style="float:right">目录</a> 
...
```
### 1.30.8. 找出所有员工当前(to_date='9999-01-01')具体的薪水salary情况，对于相同的薪水只显示一次,并按照逆序显示
<a href="#menu" style="float:right">目录</a> 
...
```
### 1.30.9. 获取所有部门当前manager的当前薪水情况，给出dept_no, emp_no以及salary，当前表示to_date='9999-01-01'
<a href="#menu" style="float:right">目录</a> 
...
### 1.30.11. 获取所有员工当前的manager，如果当前的manager是自己的话结果不显示，当前表示to_date='9999-01-01'。
...
### 1.30.12. 获取所有部门中当前员工薪水最高的相关信息，给出dept_no, emp_no以及其对应的salary
...
### 1.30.13. 从titles表获取按照title进行分组，每组个数大于等于2，给出title以及对应的数目t。
...
### 1.30.14. 从titles表获取按照title进行分组，每组个数大于等于2，给出title以及对应的数目t。
...
### 1.30.15. 查找employees表所有emp_no为奇数，且last_name不为Mary的员工信息，并按照hire_date逆序排列
...
### 1.30.16. 统计出当前各个title类型对应的员工当前薪水对应的平均工资。结果给出title以及平均工资avg。
...
### 1.30.17. 获取当前（to_date='9999-01-01'）薪水第二多的员工的emp_no以及其对应的薪水salary
...
### 1.30.18. 查找当前薪水(to_date='9999-01-01')排名第二多的员工编号emp_no、薪水salary、last_name以及first_name，不准使用order by
...
### 1.30.19. 查找所有员工的last_name和first_name以及对应的dept_name，也包括暂时没有分配部门的员工
...
### 1.30.20. 查找员工编号emp_no为10001其自入职以来的薪水salary涨幅值growth
```SQL
CREATE TABLE `salaries` (
`emp_no` int(11) NOT NULL,
`salary` int(11) NOT NULL,
...
### 1.30.21. **查找所有员工自入职以来的薪水涨幅情况，给出员工编号emp_no以及其对应的薪水涨幅growth，并按照growth进行升序
...
### 1.30.22. 统计各个部门对应员工涨幅的次数总和，给出部门编码dept_no、部门名称dept_name以及次数sum
...
### 1.30.23. 对所有员工的当前(to_date='9999-01-01')薪水按照salary进行按照1-N的排名，相同salary并列且按照emp_no升序排列
...
### 1.30.24. 获取所有非manager员工当前的薪水情况，给出dept_no、emp_no以及salary ，当前表示to_date='9999-01-01'
...
### 1.30.25. 获取员工其当前的薪水比其manager当前薪水还高的相关信息，当前表示to_date='9999-01-01',
...
### 1.30.26. 汇总各个部门当前员工的title类型的分配数目，结果给出部门编号dept_no、dept_name、其当前员工所有的title以及该类型title对应的数目count
...
### 1.30.27. 给出每个员工每年薪水涨幅超过5000的员工编号emp_no、薪水变更开始日期from_date以及薪水涨幅值salary_growth，并按照salary_growth逆序排列。
...
### 1.30.28. 查找描述信息中包括robot的电影对应的分类名称以及电影数目，而且还需要该分类对应电影数量>=5部
...
### 1.30.29. 使用join查询方式找出没有分类的电影id以及名称
...
### 1.30.30. 使用子查询的方式找出属于Action分类的所有电影对应的title,description
...
### 1.30.32. 将employees表的所有员工的last_name和first_name拼接起来作为Name，中间以一个空格区分
...
### 1.30.37. 对first_name创建唯一索引uniq_idx_firstname
```SQL
针对如下表actor结构创建索引：
CREATE TABLE IF NOT EXISTS actor (
actor_id smallint(5) NOT NULL PRIMARY KEY,
...
### 1.30.38. 针对actor表创建视图actor_name_view
```SQL
...
### 1.30.39. 针对上面的salaries表emp_no字段创建索引idx_emp_no
```SQL
...
### 1.30.40. 在last_update后面新增加一列名字为create_date
...
### 1.30.42. 删除emp_no重复的记录，只保留最小的id对应的记录。
...
### 1.30.43. 将所有to_date为9999-01-01的全部更新为NULL
```SQL
...
### 1.30.44. 将id=5以及emp_no=10001的行数据替换成id=5以及emp_no=10005
...
### 1.30.45. 将titles_test表名修改为titles_2017
```SQL
...
### 1.30.46. 在audit表上创建外键约束，其emp_no对应employees_test表的主键id
```SQL
...
### 1.30.47. 如何获取emp_v和employees有相同的数据no
...
### 1.30.49. 针对库中的所有表生成select count(*)对应的SQL语句
...
### 1.30.50. 将employees表中的所有员工的last_name和first_name通过(')连接起来。
```SQL
...
### 1.30.51. 查找字符串'10,A,B' 中逗号','出现的次数cnt
...
### 1.30.52. 获取Employees中的first_name
```SQL
...
### 1.30.54. 查找排除当前最大、最小salary之后的员工的平均工资avg_salary
```SQL
...
### 1.30.55. 分页查询employees表，每5行一页，返回第2页的数据
```SQL
...
### 1.30.57. 使用含有关键字exists查找未分配具体部门的员工的所有信息。
```SQL
...
### 1.30.58. 获取employees中的行数据，且这些行也存在于emp_v中
```SQL
存在如下的视图：
create view emp_v as select * from employees where emp_no >10005;
CREATE TABLE `employees` (
...
### 1.30.60. 统计salary的累计和running_total
```SQL
...
### 1.30.61. 对于employees表中，给出奇数行的first_name
