...
- [1. MySQL](#1-mysql)
    ...
    - [1.15. 存储引擎](#115-存储引擎)
        - [1.15.1. 常见的存储引擎](#1151-常见的存储引擎)
            - [1.15.1.1. InnoDB存储引擎](#11511-innodb存储引擎)
            - [1.15.1.2. MyISAM存储引擎](#11512-myisam存储引擎)
            - [1.15.1.3. MEMORY存储引擎](#11513-memory存储引擎)
            - [1.15.1.4. 存储引擎的选择](#11514-存储引擎的选择)
        - [1.15.2. InnoDB存储引擎](#1152-innodb存储引擎)
            - [1.15.2.1. 体系结构](#11521-体系结构)
            - [1.15.2.2. 后台线程](#11522-后台线程)
            - [1.15.2.3. 内存](#11523-内存)
            - [1.15.2.4. checkpoint技术](#11524-checkpoint技术)
            - [1.15.2.5. Master Thread 工作方式](#11525-master-thread-工作方式)
            - [1.15.2.6. InnoDB关键特性](#11526-innodb关键特性)
                - [1.15.2.6.1. 插入缓存](#115261-插入缓存)
                - [1.15.2.6.2. 两次写](#115262-两次写)
                - [1.15.2.6.3. 自适应哈希索引](#115263-自适应哈希索引)
                - [1.15.2.6.4. 异步IO](#115264-异步io)
                - [1.15.2.6.5. 刷新邻接页](#115265-刷新邻接页)
                - [1.15.2.6.6. 启动、关闭与恢复](#115266-启动关闭与恢复)
    - [1.16. MySQL基准测试](#116-mysql基准测试)
        - [1.16.1. 为什么需要基准测试](#1161-为什么需要基准测试)
        - [1.16.2. 基准测试的策略](#1162-基准测试的策略)
        - [1.16.3. 基准测试的方法](#1163-基准测试的方法)
        - [1.16.4. 基准测试工具](#1164-基准测试工具)
        - [1.16.5. 基准测试案例](#1165-基准测试案例)
    - [1.17. 服务器性能剖析](#117-服务器性能剖析)
        - [1.17.1. 性能优化简介](#1171-性能优化简介)
        - [1.17.2. 剖析MYSQL查询](#1172-剖析mysql查询)
    - [1.18. 日志文件](#118-日志文件)
        - [1.18.1. 参数文件](#1181-参数文件)
    - [1.19. 表与数据类型优化](#119-表与数据类型优化)
        - [1.19.1. 选择优化的数据类型](#1191-选择优化的数据类型)
        - [1.19.2. 表设计中的陷阱](#1192-表设计中的陷阱)
        - [1.19.3. 范式和反范式](#1193-范式和反范式)
        - [1.19.4. 加快ALTER操作的速度](#1194-加快alter操作的速度)
        - [1.19.5. 总结](#1195-总结)
        - [1.19.6. 日志文件](#1196-日志文件)
        - [1.19.7. socket文件](#1197-socket文件)
        - [1.19.8. pid文件](#1198-pid文件)
        - [1.19.9. MySQL表结构文件](#1199-mysql表结构文件)
        - [1.19.10. 存储引擎文件](#11910-存储引擎文件)
    - [1.20. 表](#120-表)
        - [1.20.1. 索引组织表](#1201-索引组织表)
        - [1.20.2. 逻辑存储结构](#1202-逻辑存储结构)
        - [1.20.3. 行记录格式](#1203-行记录格式)
        - [1.20.4. 数据页结构](#1204-数据页结构)
        - [1.20.5. Nameed File Formats机制](#1205-nameed-file-formats机制)
        - [1.20.6. 约束](#1206-约束)
        - [1.20.7. 视图](#1207-视图)
        - [1.20.8. 分区表](#1208-分区表)
    - [1.21. 索引与算法](#121-索引与算法)
        - [1.21.1. 索引概述](#1211-索引概述)
        - [1.21.2. 表和索引结构](#1212-表和索引结构)
            - [1.21.2.1. 索引页和表页](#12121-索引页和表页)
            - [1.21.2.2. 缓冲池和磁盘IO](#12122-缓冲池和磁盘io)
        - [1.21.3. B树和B+树](#1213-b树和b树)
            - [1.21.3.1. B树](#12131-b树)
            - [1.21.3.2. B+树](#12132-b树)
        - [1.21.4. B+树索引](#1214-b树索引)
            - [1.21.4.1. 聚集索引](#12141-聚集索引)
            - [1.21.4.2. 辅助索引](#12142-辅助索引)
            - [1.21.4.3. B+树索引的分裂](#12143-b树索引的分裂)
            - [1.21.4.4. B+树索引的管理](#12144-b树索引的管理)
        - [1.21.5. B+树索引的使用](#1215-b树索引的使用)
            - [1.21.5.1. 不同应用中B+树索引的使用](#12151-不同应用中b树索引的使用)
            - [1.21.5.2. 联合索引](#12152-联合索引)
            - [1.21.5.3. 覆盖索引](#12153-覆盖索引)
            - [1.21.5.4. 优化器选择不使用索引的情况](#12154-优化器选择不使用索引的情况)
            - [1.21.5.5. 索引提示](#12155-索引提示)
            - [1.21.5.6. Multi-Range Read优化](#12156-multi-range-read优化)
            - [1.21.5.7. Index Condition Pushdown 优化](#12157-index-condition-pushdown-优化)
        - [1.21.6. 哈希算法](#1216-哈希算法)
            - [1.21.6.1. InnoDB哈希算法](#12161-innodb哈希算法)
            - [1.21.6.2. 自适应哈希算法](#12162-自适应哈希算法)
        - [1.21.7. 全文索引](#1217-全文索引)
            - [1.21.7.1. 倒排索引](#12171-倒排索引)
            - [1.21.7.2. InnoDB全文索引](#12172-innodb全文索引)
            - [1.21.7.3. 全文索引](#12173-全文索引)
    - [1.22. 锁](#122-锁)
        - [1.22.1. 什么是锁](#1221-什么是锁)
            - [1.22.1.1. 锁](#12211-锁)
        - [1.22.2. lock与latch](#1222-lock与latch)
        - [1.22.3. Innodb存储引擎的锁](#1223-innodb存储引擎的锁)
            - [1.22.3.1. 锁的类型](#12231-锁的类型)
            - [1.22.3.2. 一致性非锁定读](#12232-一致性非锁定读)
            - [1.22.3.3. 一致性锁定读](#12233-一致性锁定读)
            - [1.22.3.4. 自增长与锁](#12234-自增长与锁)
            - [1.22.3.5. 外键和锁](#12235-外键和锁)
        - [1.22.4. 锁的算法](#1224-锁的算法)
            - [1.22.4.1. 行锁的三种算法](#12241-行锁的三种算法)
        - [1.22.5. 锁的问题](#1225-锁的问题)
        - [1.22.6. 阻塞](#1226-阻塞)
        - [1.22.7. 死锁](#1227-死锁)
            - [1.22.7.1. 死锁的概念](#12271-死锁的概念)
            - [1.22.7.2. 死锁的实例](#12272-死锁的实例)
        - [1.22.8. 锁升级](#1228-锁升级)
        - [1.22.9. 乐观锁和悲观锁](#1229-乐观锁和悲观锁)
            - [1.22.9.1. 多版本并发控制MVCC](#12291-多版本并发控制mvcc)
    - [1.23. 事务](#123-事务)
        - [1.23.1. 认识事务](#1231-认识事务)
            - [1.23.1.1. 事务特点](#12311-事务特点)
            - [1.23.1.2. 常见术语](#12312-常见术语)
            - [1.23.1.3. 基本语句](#12313-基本语句)
            - [1.23.1.4. 事务分类](#12314-事务分类)
        - [1.23.2. 事务的实现](#1232-事务的实现)
            - [1.23.2.1. redo](#12321-redo)
            - [1.23.2.2. undo](#12322-undo)
            - [1.23.2.3. purge](#12323-purge)
            - [1.23.2.4. group commit](#12324-group-commit)
        - [1.23.3. 事务控制语句](#1233-事务控制语句)
        - [1.23.4. 隐式提交的SQL语句](#1234-隐式提交的sql语句)
        - [1.23.5. 对于事务操作的统计](#1235-对于事务操作的统计)
        - [1.23.6. 事务的隔离级别](#1236-事务的隔离级别)
        - [1.23.7. 分布式事务](#1237-分布式事务)
            - [1.23.7.1. MYSQL分布式事务](#12371-mysql分布式事务)
            - [1.23.7.2. 内部XA事务](#12372-内部xa事务)
        - [1.23.8. 不好的事务习惯](#1238-不好的事务习惯)
    - [1.24. 备份与恢复](#124-备份与恢复)
    - [1.25. 性能优化基本的分析命令](#125-性能优化基本的分析命令)
        - [1.25.1. explain](#1251-explain)
        - [1.25.2. show indexs](#1252-show-indexs)
        - [1.25.3. show full processlist](#1253-show-full-processlist)
        - [1.25.4. show create table user](#1254-show-create-table-user)
        - [1.25.5. show table status](#1255-show-table-status)
        - [1.25.6. show global|session status](#1256-show-globalsession-status)
        - [1.25.7. show global|session  variables like "xxx"](#1257-show-globalsession--variables-like-xxx)
    - [1.26. 分库分表](#126-分库分表)
        - [1.26.1. 基本概念](#1261-基本概念)
        - [1.26.2. 分布式事务](#1262-分布式事务)
        - [1.26.3. 应对多机数据查询](#1263-应对多机数据查询)
            - [1.26.3.1. 跨库Join](#12631-跨库join)
            - [1.26.3.2. 外键约束](#12632-外键约束)
    - [1.27. 文件](#127-文件)
        - [1.27.1. 参数文件](#1271-参数文件)
        - [1.27.2. 套接字文件](#1272-套接字文件)
        - [1.27.3. pid文件](#1273-pid文件)
        - [1.27.4. 表结构定义文件](#1274-表结构定义文件)
        - [1.27.5. 日志](#1275-日志)
            - [1.27.5.1. 错误日志](#12751-错误日志)
            - [1.27.5.2. 查询日志](#12752-查询日志)
            - [1.27.5.3. 二进制日志](#12753-二进制日志)
            - [1.27.5.4. 慢查询日志](#12754-慢查询日志)
            - [1.27.5.5. 中继日志](#12755-中继日志)
            - [1.27.5.6. 事务日志](#12756-事务日志)
    - [1.28. 高可用](#128-高可用)
        - [1.28.1. 主从复制](#1281-主从复制)
            - [1.28.1.1. 复制功能概述](#12811-复制功能概述)
            - [1.28.1.2. 常见形式](#12812-常见形式)
            - [1.28.1.3. 主从同步复制原理](#12813-主从同步复制原理)
            - [1.28.1.4. 主从同步配置](#12814-主从同步配置)
            - [1.28.1.5. 添加更多从服务器](#12815-添加更多从服务器)
...
#### 1.21.5.1. 不同应用中B+树索引的使用

#### 1.21.5.2. 联合索引

#### 1.21.5.3. 覆盖索引

#### 1.21.5.4. 优化器选择不使用索引的情况

#### 1.21.5.5. 索引提示

#### 1.21.5.6. Multi-Range Read优化

#### 1.21.5.7. Index Condition Pushdown 优化
...
### 1.25.6. show global|session status
查看当前服务器内部信息。
...
### 1.25.7. show global|session  variables like "xxx"
