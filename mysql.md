...
- [1. MySQL](#1-mysql)
    ...
    - [1.16. InnoDB存储引擎](#116-innodb存储引擎)
        - [1.16.1. 体系结构](#1161-体系结构)
            - [1.16.1.1. 后台线程](#11611-后台线程)
            - [1.16.1.2. 内存](#11612-内存)
        - [1.16.2. checkpoint技术](#1162-checkpoint技术)
        - [1.16.3. Master Thread 工作方式](#1163-master-thread-工作方式)
        - [1.16.4. InnoDB关键特性](#1164-innodb关键特性)
            - [1.16.4.1. 插入缓存](#11641-插入缓存)
            - [1.16.4.2. 两次写](#11642-两次写)
            - [1.16.4.3. 自适应哈希索引](#11643-自适应哈希索引)
            - [1.16.4.4. 异步IO](#11644-异步io)
            - [1.16.4.5. 刷新邻接页](#11645-刷新邻接页)
            - [1.16.4.6. 启动、关闭与恢复](#11646-启动关闭与恢复)
    - [1.17. 日志文件](#117-日志文件)
        - [1.17.1. 参数文件](#1171-参数文件)
        - [1.17.2. 日志文件](#1172-日志文件)
        - [1.17.3. socket文件](#1173-socket文件)
        - [1.17.4. pid文件](#1174-pid文件)
        - [1.17.5. MySQL表结构文件](#1175-mysql表结构文件)
        - [1.17.6. 存储引擎文件](#1176-存储引擎文件)
    - [1.18. 表](#118-表)
    ...
    - [1.19. 索引与算法](#119-索引与算法)
        - [1.19.1. 索引概述](#1191-索引概述)
        - [1.19.2. 数据结构和算法](#1192-数据结构和算法)
        - [1.19.3. B树和B+树](#1193-b树和b树)
            - [1.19.3.1. B树](#11931-b树)
            - [1.19.3.2. B+树](#11932-b树)
        - [1.19.4. B+树索引](#1194-b树索引)
            - [1.19.4.1. 聚集索引](#11941-聚集索引)
            - [1.19.4.2. 辅助索引](#11942-辅助索引)
            - [1.19.4.3. B+树索引的分裂](#11943-b树索引的分裂)
            - [1.19.4.4. B+树索引的管理](#11944-b树索引的管理)
        - [1.19.5. B+树索引的使用](#1195-b树索引的使用)
            - [1.19.5.1. 不同应用中B+树索引的使用](#11951-不同应用中b树索引的使用)
            - [1.19.5.2. 联合索引](#11952-联合索引)
            - [1.19.5.3. 覆盖索引](#11953-覆盖索引)
            - [1.19.5.4. 优化器选择不使用索引的情况](#11954-优化器选择不使用索引的情况)
            - [1.19.5.5. 索引提示](#11955-索引提示)
            - [1.19.5.6. Multi-Range Read优化](#11956-multi-range-read优化)
            - [1.19.5.7. Index Condition Pushdown 优化](#11957-index-condition-pushdown-优化)
        - [1.19.6. 哈希算法](#1196-哈希算法)
            - [1.19.6.1. InnoDB哈希算法](#11961-innodb哈希算法)
            - [1.19.6.2. 自适应哈希算法](#11962-自适应哈希算法)
        - [1.19.7. 全文索引](#1197-全文索引)
            - [1.19.7.1. 倒排索引](#11971-倒排索引)
            - [1.19.7.2. InnoDB全文索引](#11972-innodb全文索引)
            - [1.19.7.3. 全文索引](#11973-全文索引)
    - [1.20. 锁](#120-锁)
    ...
    - [1.25. 文件](#125-文件)
        - [1.25.1. 参数文件](#1251-参数文件)
        - [1.25.2. 套接字文件](#1252-套接字文件)
        - [1.25.3. pid文件](#1253-pid文件)
        - [1.25.4. 表结构定义文件](#1254-表结构定义文件)
        - [1.25.5. 日志](#1255-日志)
            - [1.25.5.1. 错误日志](#12551-错误日志)
            - [1.25.5.2. 查询日志](#12552-查询日志)
            - [1.25.5.3. 二进制日志](#12553-二进制日志)
            - [1.25.5.4. 慢查询日志](#12554-慢查询日志)
            - [1.25.5.5. 中继日志](#12555-中继日志)
            - [1.25.5.6. 事务日志](#12556-事务日志)
    - [1.26. 高可用](#126-高可用)
        - [1.26.1. 主从复制](#1261-主从复制)
            - [1.26.1.1. 复制功能概述](#12611-复制功能概述)
            - [1.26.1.2. 常见形式](#12612-常见形式)
            - [1.26.1.3. 主从同步复制原理](#12613-主从同步复制原理)
            - [1.26.1.4. 主从同步配置](#12614-主从同步配置)
            - [1.26.1.5. 添加更多从服务器](#12615-添加更多从服务器)
    - [1.27. SQL实战](#127-sql实战)
        - [1.27.1. 查找最晚入职员工的所有信息](#1271-查找最晚入职员工的所有信息)
        - [1.27.2. 查找入职员工时间排名倒数第三的员工所有信息](#1272-查找入职员工时间排名倒数第三的员工所有信息)
        - [1.27.3. 查找各个部门当前(to_date='9999-01-01')领导当前薪水详情以及其对应部门编号dept_no](#1273-查找各个部门当前to_date9999-01-01领导当前薪水详情以及其对应部门编号dept_no)
        - [1.27.4. 查找所有已经分配部门的员工的last_name和first_name](#1274-查找所有已经分配部门的员工的last_name和first_name)
        - [1.27.5. 查找所有员工的last_name和first_name以及对应部门编号dept_no，也包括展示没有分配具体部门的员工](#1275-查找所有员工的last_name和first_name以及对应部门编号dept_no也包括展示没有分配具体部门的员工)
        - [1.27.6. 查找所有员工入职时候的薪水情况，给出emp_no以及salary， 并按照emp_no进行逆序](#1276-查找所有员工入职时候的薪水情况给出emp_no以及salary-并按照emp_no进行逆序)
        - [1.27.7. 查找薪水涨幅超过15次的员工号emp_no以及其对应的涨幅次数t](#1277-查找薪水涨幅超过15次的员工号emp_no以及其对应的涨幅次数t)
        - [1.27.8. 找出所有员工当前(to_date='9999-01-01')具体的薪水salary情况，对于相同的薪水只显示一次,并按照逆序显示](#1278-找出所有员工当前to_date9999-01-01具体的薪水salary情况对于相同的薪水只显示一次并按照逆序显示)
        - [1.27.9. 获取所有部门当前manager的当前薪水情况，给出dept_no, emp_no以及salary，当前表示to_date='9999-01-01'](#1279-获取所有部门当前manager的当前薪水情况给出dept_no-emp_no以及salary当前表示to_date9999-01-01)
        - [1.27.10. 获取所有非manager的员工emp_no](#12710-获取所有非manager的员工emp_no)
        - [1.27.11. 获取所有员工当前的manager，如果当前的manager是自己的话结果不显示，当前表示to_date='9999-01-01'。](#12711-获取所有员工当前的manager如果当前的manager是自己的话结果不显示当前表示to_date9999-01-01)
        - [1.27.12. 获取所有部门中当前员工薪水最高的相关信息，给出dept_no, emp_no以及其对应的salary](#12712-获取所有部门中当前员工薪水最高的相关信息给出dept_no-emp_no以及其对应的salary)
        - [1.27.13. 从titles表获取按照title进行分组，每组个数大于等于2，给出title以及对应的数目t。](#12713-从titles表获取按照title进行分组每组个数大于等于2给出title以及对应的数目t)
        - [1.27.14. 从titles表获取按照title进行分组，每组个数大于等于2，给出title以及对应的数目t。](#12714-从titles表获取按照title进行分组每组个数大于等于2给出title以及对应的数目t)
        - [1.27.15. 查找employees表所有emp_no为奇数，且last_name不为Mary的员工信息，并按照hire_date逆序排列](#12715-查找employees表所有emp_no为奇数且last_name不为mary的员工信息并按照hire_date逆序排列)
        - [1.27.16. 统计出当前各个title类型对应的员工当前薪水对应的平均工资。结果给出title以及平均工资avg。](#12716-统计出当前各个title类型对应的员工当前薪水对应的平均工资结果给出title以及平均工资avg)
        - [1.27.17. 获取当前（to_date='9999-01-01'）薪水第二多的员工的emp_no以及其对应的薪水salary](#12717-获取当前to_date9999-01-01薪水第二多的员工的emp_no以及其对应的薪水salary)
        - [1.27.18. 查找当前薪水(to_date='9999-01-01')排名第二多的员工编号emp_no、薪水salary、last_name以及first_name，不准使用order by](#12718-查找当前薪水to_date9999-01-01排名第二多的员工编号emp_no薪水salarylast_name以及first_name不准使用order-by)
        - [1.27.19. 查找所有员工的last_name和first_name以及对应的dept_name，也包括暂时没有分配部门的员工](#12719-查找所有员工的last_name和first_name以及对应的dept_name也包括暂时没有分配部门的员工)
        - [1.27.20. 查找员工编号emp_no为10001其自入职以来的薪水salary涨幅值growth](#12720-查找员工编号emp_no为10001其自入职以来的薪水salary涨幅值growth)
        - [1.27.21. **查找所有员工自入职以来的薪水涨幅情况，给出员工编号emp_no以及其对应的薪水涨幅growth，并按照growth进行升序](#12721-查找所有员工自入职以来的薪水涨幅情况给出员工编号emp_no以及其对应的薪水涨幅growth并按照growth进行升序)
        - [1.27.22. 统计各个部门对应员工涨幅的次数总和，给出部门编码dept_no、部门名称dept_name以及次数sum](#12722-统计各个部门对应员工涨幅的次数总和给出部门编码dept_no部门名称dept_name以及次数sum)
        - [1.27.23. 对所有员工的当前(to_date='9999-01-01')薪水按照salary进行按照1-N的排名，相同salary并列且按照emp_no升序排列](#12723-对所有员工的当前to_date9999-01-01薪水按照salary进行按照1-n的排名相同salary并列且按照emp_no升序排列)
        - [1.27.24. 获取所有非manager员工当前的薪水情况，给出dept_no、emp_no以及salary ，当前表示to_date='9999-01-01'](#12724-获取所有非manager员工当前的薪水情况给出dept_noemp_no以及salary-当前表示to_date9999-01-01)
        - [1.27.25. 获取员工其当前的薪水比其manager当前薪水还高的相关信息，当前表示to_date='9999-01-01',](#12725-获取员工其当前的薪水比其manager当前薪水还高的相关信息当前表示to_date9999-01-01)
        - [1.27.26. 汇总各个部门当前员工的title类型的分配数目，结果给出部门编号dept_no、dept_name、其当前员工所有的title以及该类型title对应的数目count](#12726-汇总各个部门当前员工的title类型的分配数目结果给出部门编号dept_nodept_name其当前员工所有的title以及该类型title对应的数目count)
        - [1.27.27. 给出每个员工每年薪水涨幅超过5000的员工编号emp_no、薪水变更开始日期from_date以及薪水涨幅值salary_growth，并按照salary_growth逆序排列。](#12727-给出每个员工每年薪水涨幅超过5000的员工编号emp_no薪水变更开始日期from_date以及薪水涨幅值salary_growth并按照salary_growth逆序排列)
        - [1.27.28. 查找描述信息中包括robot的电影对应的分类名称以及电影数目，而且还需要该分类对应电影数量>=5部](#12728-查找描述信息中包括robot的电影对应的分类名称以及电影数目而且还需要该分类对应电影数量5部)
        - [1.27.29. 使用join查询方式找出没有分类的电影id以及名称](#12729-使用join查询方式找出没有分类的电影id以及名称)
        - [1.27.30. 使用子查询的方式找出属于Action分类的所有电影对应的title,description](#12730-使用子查询的方式找出属于action分类的所有电影对应的titledescription)
        - [1.27.31. 获取select](#12731-获取select)
        - [1.27.32. 将employees表的所有员工的last_name和first_name拼接起来作为Name，中间以一个空格区分](#12732-将employees表的所有员工的last_name和first_name拼接起来作为name中间以一个空格区分)
        - [1.27.33. 创建一个actor表，包含如下列信息](#12733-创建一个actor表包含如下列信息)
        - [1.27.34. 批量插入数据](#12734-批量插入数据)
        - [1.27.35. 批量插入数据，不使用replace操作](#12735-批量插入数据不使用replace操作)
        - [1.27.36. 创建一个actor_name表](#12736-创建一个actor_name表)
        - [1.27.37. 对first_name创建唯一索引uniq_idx_firstname](#12737-对first_name创建唯一索引uniq_idx_firstname)
        - [1.27.38. 针对actor表创建视图actor_name_view](#12738-针对actor表创建视图actor_name_view)
        - [1.27.39. 针对上面的salaries表emp_no字段创建索引idx_emp_no](#12739-针对上面的salaries表emp_no字段创建索引idx_emp_no)
        - [1.27.40. 在last_update后面新增加一列名字为create_date](#12740-在last_update后面新增加一列名字为create_date)
        - [1.27.41. 构造一个触发器audit_log](#12741-构造一个触发器audit_log)
        - [1.27.42. 删除emp_no重复的记录，只保留最小的id对应的记录。](#12742-删除emp_no重复的记录只保留最小的id对应的记录)
        - [1.27.43. 将所有to_date为9999-01-01的全部更新为NULL](#12743-将所有to_date为9999-01-01的全部更新为null)
        - [1.27.44. 将id=5以及emp_no=10001的行数据替换成id=5以及emp_no=10005](#12744-将id5以及emp_no10001的行数据替换成id5以及emp_no10005)
        - [1.27.45. 将titles_test表名修改为titles_2017](#12745-将titles_test表名修改为titles_2017)
        - [1.27.46. 在audit表上创建外键约束，其emp_no对应employees_test表的主键id](#12746-在audit表上创建外键约束其emp_no对应employees_test表的主键id)
        - [1.27.47. 如何获取emp_v和employees有相同的数据no](#12747-如何获取emp_v和employees有相同的数据no)
        - [1.27.48. 将所有获取奖金的员工当前的薪水增加百分之十](#12748-将所有获取奖金的员工当前的薪水增加百分之十)
        - [1.27.49. 针对库中的所有表生成select count(*)对应的SQL语句](#12749-针对库中的所有表生成select-count对应的sql语句)
        - [1.27.50. 将employees表中的所有员工的last_name和first_name通过(')连接起来。](#12750-将employees表中的所有员工的last_name和first_name通过连接起来)
        - [1.27.51. 查找字符串'10,A,B' 中逗号','出现的次数cnt](#12751-查找字符串10ab-中逗号出现的次数cnt)
        - [1.27.52. 获取Employees中的first_name](#12752-获取employees中的first_name)
        - [1.27.53. 按照dept_no进行汇总](#12753-按照dept_no进行汇总)
        - [1.27.54. 查找排除当前最大、最小salary之后的员工的平均工资avg_salary](#12754-查找排除当前最大最小salary之后的员工的平均工资avg_salary)
        - [1.27.55. 分页查询employees表，每5行一页，返回第2页的数据](#12755-分页查询employees表每5行一页返回第2页的数据)
        - [1.27.56. 获取所有员工的emp_no](#12756-获取所有员工的emp_no)
        - [1.27.57. 使用含有关键字exists查找未分配具体部门的员工的所有信息。](#12757-使用含有关键字exists查找未分配具体部门的员工的所有信息)
        - [1.27.58. 获取employees中的行数据，且这些行也存在于emp_v中](#12758-获取employees中的行数据且这些行也存在于emp_v中)
        - [1.27.59. 获取有奖金的员工相关信息。](#12759-获取有奖金的员工相关信息)
        - [1.27.60. 统计salary的累计和running_total](#12760-统计salary的累计和running_total)
        - [1.27.61. 对于employees表中，给出奇数行的first_name](#12761-对于employees表中给出奇数行的first_name)
...
#### 1.19.5.1. 不同应用中B+树索引的使用

#### 1.19.5.2. 联合索引

#### 1.19.5.3. 覆盖索引

#### 1.19.5.4. 优化器选择不使用索引的情况

#### 1.19.5.5. 索引提示

#### 1.19.5.6. Multi-Range Read优化

#### 1.19.5.7. Index Condition Pushdown 优化
...
### 1.27.3. 查找各个部门当前(to_date='9999-01-01')领导当前薪水详情以及其对应部门编号dept_no
...
### 1.27.4. 查找所有已经分配部门的员工的last_name和first_name
<a href="#menu" style="float:right">目录</a> 
...
### 1.27.5. 查找所有员工的last_name和first_name以及对应部门编号dept_no，也包括展示没有分配具体部门的员工
<a href="#menu" style="float:right">目录</a> 
...
### 1.27.6. 查找所有员工入职时候的薪水情况，给出emp_no以及salary， 并按照emp_no进行逆序
<a href="#menu" style="float:right">目录</a> 
...
### 1.27.7. 查找薪水涨幅超过15次的员工号emp_no以及其对应的涨幅次数t
<a href="#menu" style="float:right">目录</a> 
...
```
### 1.27.8. 找出所有员工当前(to_date='9999-01-01')具体的薪水salary情况，对于相同的薪水只显示一次,并按照逆序显示
<a href="#menu" style="float:right">目录</a> 
...
```
### 1.27.9. 获取所有部门当前manager的当前薪水情况，给出dept_no, emp_no以及salary，当前表示to_date='9999-01-01'
<a href="#menu" style="float:right">目录</a> 
...
### 1.27.11. 获取所有员工当前的manager，如果当前的manager是自己的话结果不显示，当前表示to_date='9999-01-01'。
...
### 1.27.12. 获取所有部门中当前员工薪水最高的相关信息，给出dept_no, emp_no以及其对应的salary
...
### 1.27.13. 从titles表获取按照title进行分组，每组个数大于等于2，给出title以及对应的数目t。
...
### 1.27.14. 从titles表获取按照title进行分组，每组个数大于等于2，给出title以及对应的数目t。
...
### 1.27.15. 查找employees表所有emp_no为奇数，且last_name不为Mary的员工信息，并按照hire_date逆序排列
...
### 1.27.16. 统计出当前各个title类型对应的员工当前薪水对应的平均工资。结果给出title以及平均工资avg。
...
### 1.27.17. 获取当前（to_date='9999-01-01'）薪水第二多的员工的emp_no以及其对应的薪水salary
...
### 1.27.18. 查找当前薪水(to_date='9999-01-01')排名第二多的员工编号emp_no、薪水salary、last_name以及first_name，不准使用order by
...
### 1.27.19. 查找所有员工的last_name和first_name以及对应的dept_name，也包括暂时没有分配部门的员工
...
### 1.27.20. 查找员工编号emp_no为10001其自入职以来的薪水salary涨幅值growth
```SQL
CREATE TABLE `salaries` (
`emp_no` int(11) NOT NULL,
`salary` int(11) NOT NULL,
...
### 1.27.21. **查找所有员工自入职以来的薪水涨幅情况，给出员工编号emp_no以及其对应的薪水涨幅growth，并按照growth进行升序
...
### 1.27.22. 统计各个部门对应员工涨幅的次数总和，给出部门编码dept_no、部门名称dept_name以及次数sum
...
### 1.27.23. 对所有员工的当前(to_date='9999-01-01')薪水按照salary进行按照1-N的排名，相同salary并列且按照emp_no升序排列
...
### 1.27.24. 获取所有非manager员工当前的薪水情况，给出dept_no、emp_no以及salary ，当前表示to_date='9999-01-01'
...
### 1.27.25. 获取员工其当前的薪水比其manager当前薪水还高的相关信息，当前表示to_date='9999-01-01',
...
### 1.27.26. 汇总各个部门当前员工的title类型的分配数目，结果给出部门编号dept_no、dept_name、其当前员工所有的title以及该类型title对应的数目count
...
### 1.27.27. 给出每个员工每年薪水涨幅超过5000的员工编号emp_no、薪水变更开始日期from_date以及薪水涨幅值salary_growth，并按照salary_growth逆序排列。
...
### 1.27.28. 查找描述信息中包括robot的电影对应的分类名称以及电影数目，而且还需要该分类对应电影数量>=5部
...
### 1.27.29. 使用join查询方式找出没有分类的电影id以及名称
...
### 1.27.30. 使用子查询的方式找出属于Action分类的所有电影对应的title,description
...
### 1.27.32. 将employees表的所有员工的last_name和first_name拼接起来作为Name，中间以一个空格区分
...
### 1.27.37. 对first_name创建唯一索引uniq_idx_firstname
```SQL
针对如下表actor结构创建索引：
CREATE TABLE IF NOT EXISTS actor (
actor_id smallint(5) NOT NULL PRIMARY KEY,
...
### 1.27.38. 针对actor表创建视图actor_name_view
```SQL
...
### 1.27.39. 针对上面的salaries表emp_no字段创建索引idx_emp_no
```SQL
...
### 1.27.40. 在last_update后面新增加一列名字为create_date
...
### 1.27.42. 删除emp_no重复的记录，只保留最小的id对应的记录。
...
### 1.27.43. 将所有to_date为9999-01-01的全部更新为NULL
```SQL
...
### 1.27.44. 将id=5以及emp_no=10001的行数据替换成id=5以及emp_no=10005
...
### 1.27.45. 将titles_test表名修改为titles_2017
```SQL
...
### 1.27.46. 在audit表上创建外键约束，其emp_no对应employees_test表的主键id
```SQL
...
### 1.27.47. 如何获取emp_v和employees有相同的数据no
...
### 1.27.49. 针对库中的所有表生成select count(*)对应的SQL语句
...
### 1.27.50. 将employees表中的所有员工的last_name和first_name通过(')连接起来。
```SQL
...
### 1.27.51. 查找字符串'10,A,B' 中逗号','出现的次数cnt
...
### 1.27.52. 获取Employees中的first_name
```SQL
...
### 1.27.54. 查找排除当前最大、最小salary之后的员工的平均工资avg_salary
```SQL
...
### 1.27.55. 分页查询employees表，每5行一页，返回第2页的数据
```SQL
...
### 1.27.57. 使用含有关键字exists查找未分配具体部门的员工的所有信息。
```SQL
...
### 1.27.58. 获取employees中的行数据，且这些行也存在于emp_v中
```SQL
存在如下的视图：
create view emp_v as select * from employees where emp_no >10005;
CREATE TABLE `employees` (
...
### 1.27.60. 统计salary的累计和running_total
```SQL
...
### 1.27.61. 对于employees表中，给出奇数行的first_name
