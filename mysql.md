...
- [1. MySQL](#1-mysql)
  ...
  - [* 在解析查询之前，服务器会先检察查询缓存(如果开启的话)，存在缓存则直接将缓存返回客户端，更新操作将会修改缓存，缓存中没有数据，查询之后也会更新缓存。](#-在解析查询之前服务器会先检察查询缓存如果开启的话存在缓存则直接将缓存返回客户端更新操作将会修改缓存缓存中没有数据查询之后也会更新缓存)
  ...
  - [1.13. 存储引擎](#113-存储引擎)
    ...
    - [1.13.2. InnoDB存储引擎](#1132-innodb存储引擎)
      ...
      - [1.13.2.4. checkpoint技术](#11324-checkpoint技术)
  ...
  - [1.30. 数据库连接池](#130-数据库连接池)
    ...
    - [1.30.3. 使用数据库连接池的关键点](#1303-使用数据库连接池的关键点)
    - [1.30.4. 使用数据库连接池的优势和其工作原理](#1304-使用数据库连接池的优势和其工作原理)
    - [1.30.5. 数据库连接池druid](#1305-数据库连接池druid)
...
#### 1.19.6.6. Multi-Range Read优化

#### 1.19.6.7. Index Condition Pushdown 优化
...
### 1.23.6. show global|session status
查看当前服务器内部信息。
...
### 1.23.7. show global|session  variables like "xxx"
...
#### 1.25.7.2. 主动-主动模式下的主-从复制
<a href="#menu" >目录</a>

#### 1.25.7.3. 主动-被动模式下的主-从复制
<a href="#menu" >目录</a>
...
#### 1.25.8.1. 为什么复制无法扩展写操作
<a href="#menu" >目录</a>


#### 1.25.8.2. 备库什么时候开始写延迟
<a href="#menu" >目录</a>
...
#### 1.25.10.3. 混合事务型和非事务型表
<a href="#menu" >目录</a>

#### 1.25.10.4. 不确定语句
<a href="#menu" >目录</a>

#### 1.25.10.5. 主库和备库使用不同的存储引擎
<a href="#menu" >目录</a>
...
#### 1.25.10.12. Innodb加锁读引起的锁争用
<a href="#menu" >目录</a>

#### 1.25.10.13. 在主主复制结构中写入两台主库
<a href="#menu" >目录</a>
