...
- [1. 分布式存储系统](#1-分布式存储系统)
    - [1.1. 概述](#11-概述)
    - [1.2. 存储系统](#12-存储系统)
        - [1.2.2. 单机存储引擎](#122-单机存储引擎)
        - [1.2.3. 数据模型](#123-数据模型)
        - [1.2.4. 事务和并发控制](#124-事务和并发控制)
        - [1.2.5. 故障恢复](#125-故障恢复)
        - [1.2.6. 数据压缩](#126-数据压缩)
    - [1.3. 分布式文件系统](#13-分布式文件系统)
        - [1.3.1. 概述](#131-概述)
        - [1.3.2. 文件系统的特点](#132-文件系统的特点)
            - [1.3.2.1. 单机时代](#1321-单机时代)
            - [1.3.2.2. 独立文件服务器](#1322-独立文件服务器)
            - [1.3.2.3. 分布式文件系统](#1323-分布式文件系统)
        - [1.3.3. FastDFS](#133-fastdfs)
            - [1.3.3.1. 基本概念](#1331-基本概念)
                - [1.3.3.1.1. 什么是FastDFS](#13311-什么是fastdfs)
            - [1.3.3.2. 工作流程](#1332-工作流程)
                - [1.3.3.2.1. 文件上传](#13321-文件上传)
                - [1.3.3.2.2. 文件下载](#13322-文件下载)
            - [1.3.3.3. FastDFS为什么要结合Nginx](#1333-fastdfs为什么要结合nginx)
            - [1.3.3.4. 配置文件说明](#1334-配置文件说明)
            - [1.3.3.5. FastDFS 同步机制说明：](#1335-fastdfs-同步机制说明)
            - [1.3.3.6. 安装配置](#1336-安装配置)
        - [1.3.4. Java客户端使用](#134-java客户端使用)
...
* 定义
    * 分布式存储系统是大量普通的计算机服务器通过网络互联，对外作为一个整体提供存储服务
* 特性
    * 可扩展
        * 分布式存储系统可以扩展到成百上千台的集群规模，并且随着集群规模的增加，系统的整体性能表现也是线性增加
    * 低成本
        * 分布式存储系统的自动容错，自动负载均衡机制使其可以构建在普通的计算机上
    * 高性能
    * 易用
        * 提供易用的接口，具备完善的监控运维工具，能够方便地与其他系统集成

* 技术挑战
    * 数据分布
        * 如何将数据分布到多台服务器才能保证数据分布均匀？数据分布到多台机器后如何实现跨服务器的读写操作
    * 一致性
        * 如何将数据的多个副本复制到多台机器，即使在异常的状况下，也能保证不同副本之间数据的一致性
    * 容错
        * 如何检测到服务器故障，如何自动将出现故障的服务器上的数据和服务迁移到集群中的其他服务器
    * 负载均衡
        * 新增服务器和集群正常运行过程中如何实现自动负载均衡，数据迁移的过程中如何保证不影响已有的服务
    * 事务和并发控制
...
* 数据类型
    * 非结构化数据
        * 包括所有格式的办公文档，文本图像视频等
    * 结构化数据
        * 一般存储在关系数据库中，可以用二维关系表来表示．结构化数据的模式(schema,包括属性，数据类型以及数据之间的联系)和内容是分开的，数据的模型需要事先定义
    * 半结构化数据
        * 介于结构化和非结构化数据之间．比如html．半结构化数据的模型和内容是混在一起的，也不需要预先定义．　

* 分布式存储系统分类
    * 分布式文件系统
        * 互联网应用需要存储大量的图片，照片视频等非结构化数据对象，对象之间没有关联，这类数据一般称为ＢLOG（binary large Object）数据,
        * 用于存储非结构化的数据
    * 分布式键值系统
        * 用于存储关系简单的半结构化数据
    * 分布式表格系统
...
相比哈希存储引擎，b树存储引擎不仅支持随机读取，还支持范围扫描．关系数据库中通过索引访问数据，innodb中，有一个称为聚集索引的特殊索引，行的数据位于其中，组织成Ｂ＋树数据结构．

![mysql-b+tree](pic/distribution/store/b+tree.png)
如图，Innodb按照页面Page来组织数据，每个页面对应B+树上的一个节点．其中，叶子节点保存每行的完整数据，非叶子节点保存索引信息．数据在每个节点中有序存储．数据库查询需要从根节点开始二分查找叶子节点，每次读取一个节点，如果对应的页面不在内存中，需要从磁盘中读取并缓存起来．B+树的根节点是常驻内存的，因此B+树一次检索需要h-1次磁盘IO，复杂度为O(h)=O($\log_d N$)．(N为元素个数,d为每个节点的出度,h为B+树高度).修改操作首先需要记录提交日志,接着修改内存中的B+树.如果内存中的被修改的页面超过一定的比率,后台线程会将这些页面刷新到磁盘中持久化.
...
文件系统负责文件的组织，存储，检索，命名，共享和保护．它提供了描述文件抽象的程序接口．这样程序员就不必关心存储分配和存储布局的细节，文件存储在磁盘或其他稳定的存储介质上

文件包括数据和属性，属性包括文件长度，时间戳，文件类型，拥有者身份和访问控制列表等信息．文件中的元数据是指文件系统用于管理文件而存储的所有额外信息，包括文件属性，目录和其他文件系统使用的持久信息．
...
分布式文件系统使程序可以像对本地文件那样对远程文件进行存储和访问，允许用户访问网络中的任意计算机上的文件．访问存储在服务器上的文件应该能获得与访问本地磁盘文件类似的性能和可靠性．

* 分布式文件系统的需求
    * 透明性
        * 访问透明性
            * 客户程序应该不了解文件的分布性，用户通过一组文件操作来访问本地或者远程文件．操作本地文件的程序在不作修改的情况下应该能访问远程文件
        * 位置透明性
            * 客户程序应该使用单一的文件命名空间．在不改变路径的情况下．，多个文件或者文件组应该可以被重新定位．同时应用程序在任意时刻执行时都使用同样的名字空间
        * 移动透明性
            * 当文件被移动时，客户程序和客户节点上的系统管理表都不必进行修改，它们支持文件的移动性．
        * 性能透明性
            * 当负载在一定范围内变化时，能获得较好的访问性能
        * 伸缩透明性
            * 文件服务可以不断扩充，以满足负载和网络规模增长的需要
    * 并发文件更新
    * 文件复制
    * 硬件和操作系统异构性
    * 容错
    ...
    * 安全性
    * 效率
        * 分布式文件系统应该提供至少和传统文件系统相同的能力，并且还应满足一定的性能要求．
### 1.3.3. FastDFS
