...
public class TokenBucketLimit {
    private static ConcurrentHashMap<String,TokenBucket> map = new ConcurrentHashMap<>();
    ...
    private class TokenBucket{
        ...
        public TokenBucket(int cap,int count,int persecond) {
            this.cap = cap;
            this.remaind = cap;
    ...
    public boolean isAllow(String name ,String nextKey,int cap,int count,int persecons){
        ...
        if(null == map.get(key)){
            map.put(key,new TokenBucket(cap,count,persecons));
        }



        TokenBucket leakyBucket = map.get(key);
