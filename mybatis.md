...
* 使用${}时，statementType只能定义为STATEMENT，而#{}可以定义为所有类型(STATEMENT|PREPARED|CALLABLE)
...
Mybatis使用RowBounds对象进行分页，它是针对ResultSet结果集执行的内存分页，而非物理分页。由于这种方式会将所有数据查询出来，然后在内存进行分页。如果数据量大，可能会造成内存溢出。可以在sql内直接书写带有物理分页的参数来完成物理分页功能，也可以使用分页插件来完成物理分页。
...
mybatis有EnumTypeHandler类型处理器，完成java的枚举类型和数据库字符串类型的转换。
...
-->
  <resultMap id="BaseResultMap" type="com.springboot.mybatis.model.SysUser">
    <!--主键定义映射-->
  ...
  <insert id="insert" parameterType="com.springboot.mybatis.model.SysUser">
    <selectKey keyProperty="id" order="AFTER" resultType="java.lang.Long">
      SELECT LAST_INSERT_ID()
    </selectKey>
    insert into sys_user (user_name, user_password, user_email, 
      head_img, create_time, sex, 
      gender, user_info)
    values (#{userName,jdbcType=VARCHAR}, #{userPassword,jdbcType=VARCHAR}, #{userEmail,jdbcType=VARCHAR}, 
      #{headImg,jdbcType=VARCHAR}, #{createTime,jdbcType=TIMESTAMP}, #{sex,jdbcType=SMALLINT}, 
      #{gender,jdbcType=CHAR}, #{userInfo,jdbcType=LONGVARCHAR})
  </insert>
  <update id="updateByPrimaryKey" parameterType="com.springboot.mybatis.model.SysUser">
    update sys_user
  ...
  <select id="selectByPrimaryKey" parameterType="java.lang.Long" resultMap="BaseResultMap">
    select id, user_name, user_password, user_email, head_img, create_time, sex, gender, 
    user_info
    from sys_user
    where id = #{id,jdbcType=BIGINT}
  </select>
...
如果数据库id字段是int类型，那么它的jdbc就是Integer类型。当实体类的这个映射属性id为Long类型时，如果不设置jdbcType和javaType的话，查询的结果返回给实体时就会转换错误，写了这两个mybatis就会帮我们转换成相应的类型，从来避免发生错误。也就是说mybatis会根据javaType类型选择普通的类型处理器TypeHandler。如果java类型是常规类型，比如字符串，枚举，整型等可以不用特别设置，java会自行判断。其他类型则需要进行设置。
...
![SQL执行流程](pic/mybatis/mybatis-excute-sql.png)
...
insert into student values('1','admin','MALE');
insert into student values('2','test','FEMALE');
```
实体类
```java
...
```
mybatis会自动检测gender是枚举类，会自动使用枚举类型处理handler：EnumTypeHandler
```xml
 <resultMap id="BaseResultMap" type="com.springboot.mybatis.model.Student">
    <id column="id" jdbcType="BIGINT" property="id" />
    <result column="name" jdbcType="VARCHAR" property="name" />
    <result column="gender" jdbcType="CHAR" property="gender" />
  </resultMap>
