...
网络通信中同步机制和异步机制是描述通信模式的概念。同步机制指发送方发送请求后，需要等到接收方返回的响应，才接着发送下一个请求。异步机制是不用等待响应就发送下一个请求。
在同步机制中，所有的请求在服务器端得到同步，发送方和接收方请求的处理步调是一样的。在异步机制中，所有来自发送方的请求形成一个队列，接收方处理完成后通知发送方。
