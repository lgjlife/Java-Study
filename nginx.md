...
- [1. Nginx](#1-nginx)
    ...
    - [1.2. 安装和使用](#12-安装和使用)
        - [1.2.1. 使用包管理器安装](#121-使用包管理器安装)
        - [1.2.2. 使用二进制文件安装](#122-使用二进制文件安装)
        - [1.2.3. docker 方式安装](#123-docker-方式安装)
        - [1.2.4. configure配置说明](#124-configure配置说明)
            - [1.2.4.1. 常见配置说明](#1241-常见配置说明)
            - [1.2.4.2. Nginx重新编译添加模块的方法](#1242-nginx重新编译添加模块的方法)
            - [1.2.4.3. 安装第三方模块](#1243-安装第三方模块)
            - [1.2.4.4. 常用命令](#1244-常用命令)
    - [1.3. 配置文件](#13-配置文件)
        - [1.3.1. 内核参数优化](#131-内核参数优化)
        - [1.3.2. Nginx中的进程](#132-nginx中的进程)
        - [1.3.3. 配置文件结构](#133-配置文件结构)
        - [1.3.4. 全局配置参数](#134-全局配置参数)
            - [1.3.4.1. 常用配置](#1341-常用配置)
            - [1.3.4.2. 日志文件](#1342-日志文件)
                - [1.3.4.2.1. 访问日志](#13421-访问日志)
                - [1.3.4.2.2. 错误日志](#13422-错误日志)
                - [1.3.4.2.3. 日志文件切割](#13423-日志文件切割)
        - [1.3.5. 虚拟主机server配置](#135-虚拟主机server配置)
            - [1.3.5.1. 配置网络监听](#1351-配置网络监听)
            - [1.3.5.2. 访问权限控制](#1352-访问权限控制)
            - [1.3.5.3. 基于名称的虚拟主机配置](#1353-基于名称的虚拟主机配置)
            - [1.3.5.4. 基于端口配置虚拟主机](#1354-基于端口配置虚拟主机)
            - [1.3.5.5. 基于IP配置虚拟主机](#1355-基于ip配置虚拟主机)
            - [1.3.5.6. 基于域名配置虚拟主机](#1356-基于域名配置虚拟主机)
            - [1.3.5.7. 自定义错误页](#1357-自定义错误页)
        - [1.3.6. 其他配置](#136-其他配置)
        - [1.3.7. 目录设置](#137-目录设置)
            - [1.3.7.1. root配置](#1371-root配置)
            - [1.3.7.2. alias配置](#1372-alias配置)
        - [1.3.8. 配置location块匹配](#138-配置location块匹配)
        - [1.3.9. 网页压缩传输](#139-网页压缩传输)
        - [1.3.10. 重写和重定向](#1310-重写和重定向)
        - [1.3.11. 防盗链配置](#1311-防盗链配置)
            - [1.3.11.1. 图片防盗链](#13111-图片防盗链)
            - [1.3.11.2. 下载防盗链](#13112-下载防盗链)
        - [1.3.12. 配置https网站](#1312-配置https网站)
    - [1.4. 代理服务](#14-代理服务)
        - [1.4.1. 正向代理和反向代理区别](#141-正向代理和反向代理区别)
        - [1.4.2. Nginx正向代理服务](#142-nginx正向代理服务)
        - [1.4.3. 反向代理服务](#143-反向代理服务)
            - [1.4.3.1. 常用配置指令](#1431-常用配置指令)
                - [1.4.3.1.1. 反向代理基本设置的21个指令](#14311-反向代理基本设置的21个指令)
                - [1.4.3.1.2. proxy buffer 的配置的7个指令](#14312-proxy-buffer-的配置的7个指令)
                - [1.4.3.1.3. proxy cache 的配置的12个指令](#14313-proxy-cache-的配置的12个指令)
        - [1.4.4. 负载均衡](#144-负载均衡)
            - [1.4.4.1. 基本理论](#1441-基本理论)
            - [1.4.4.2. 配置](#1442-配置)
    - [1.5. 缓存机制](#15-缓存机制)
        - [1.5.1. 缓存实现原理](#151-缓存实现原理)
        - [1.5.2. 永久缓存配置](#152-永久缓存配置)
        - [1.5.3. 临时缓存配置](#153-临时缓存配置)
        - [1.5.4. 缓存清理配置](#154-缓存清理配置)
    - [1.6. nginx中集成lua](#16-nginx中集成lua)
    - [1.7. 高级配置](#17-高级配置)
    - [1.8. gzip压缩](#18-gzip压缩)
    - [1.9. rewrite功能](#19-rewrite功能)
        - [1.9.1. nginx后端服务器组的配置的5个技术指令](#191-nginx后端服务器组的配置的5个技术指令)
        - [1.9.2. rewrite功能的配置](#192-rewrite功能的配置)
        - [1.9.3. rewrite使用](#193-rewrite使用)
    - [1.10. 邮件服务](#110-邮件服务)
    - [1.11. 架构说明](#111-架构说明)
        - [1.11.1. 模块化结构](#1111-模块化结构)
            - [1.11.1.1. 模块分类](#11111-模块分类)
        - [1.11.2. web请求处理机制](#1112-web请求处理机制)
        - [1.11.3. 设计架构概览](#1113-设计架构概览)
    - [1.12. 工作进程](#112-工作进程)
        - [1.12.1. nginx服务进程间通信](#1121-nginx服务进程间通信)
            - [1.12.1.1. Linux进程间通信方式](#11211-linux进程间通信方式)
            - [1.12.1.2. Linux进程间双工通信方式的实现](#11212-linux进程间双工通信方式的实现)
            - [1.12.1.3. 通信通道建立和设置](#11213-通信通道建立和设置)
            - [1.12.1.4. 通信通道的使用](#11214-通信通道的使用)
            - [1.12.1.5. 消息的读写](#11215-消息的读写)
    - [1.13. 模块编程](#113-模块编程)
        - [1.13.1. 模块的种类](#1131-模块的种类)
        - [1.13.2. 模块开发实战](#1132-模块开发实战)
        - [1.13.3. 模块的编译和安装](#1133-模块的编译和安装)
    - [1.14. nginx 优化](#114-nginx-优化)
        - [1.14.1. Nginx运行工作进程数量](#1141-nginx运行工作进程数量)
        - [1.14.2. Nginx最大打开文件数](#1142-nginx最大打开文件数)
        - [1.14.3. Nginx事件处理模型](#1143-nginx事件处理模型)
        - [1.14.4. 开启高效传输模式](#1144-开启高效传输模式)
        - [1.14.5. 连接超时时间](#1145-连接超时时间)
        - [1.14.6. fastcgi 调优](#1146-fastcgi-调优)
        - [1.14.7. gzip 调优](#1147-gzip-调优)
        - [1.14.8. expires 缓存调优](#1148-expires-缓存调优)
        - [1.14.9. 防盗链](#1149-防盗链)
        - [1.14.10. 内核参数优化](#11410-内核参数优化)
    - [1.15. nginx源码结构](#115-nginx源码结构)
        - [1.15.1. 目录结构](#1151-目录结构)
        - [1.15.2. 模块结构](#1152-模块结构)
    - [1.16. 启动初始化](#116-启动初始化)
        - [1.16.1. 启动过程概览](#1161-启动过程概览)
    - [1.17. 时间管理](#117-时间管理)
    - [1.18. 内存管理](#118-内存管理)
        - [1.18.1. 内存的逻辑结构](#1181-内存的逻辑结构)
        - [1.18.2. 内存池管理](#1182-内存池管理)
        - [1.18.3. 内存的使用](#1183-内存的使用)
...
```
##### 1.4.3.1.2. proxy buffer 的配置的7个指令
<a href="#menu" >目录</a>
...
##### 1.4.3.1.3. proxy cache 的配置的12个指令
<a href="#menu" >目录</a>
