...
Java使用unicode表示字符，unicode定义了一个完全国际化的字符集，能表示全部人类语言中的所有字符。是多种字符集的集合体。
...
读取和写入文件的I/O操作都是通过操作系统所提供的接口，因为磁盘是由操作系统管理的。读和写分别通过read和write来实现。每次调用都涉及到用户地址空间和内核地址空间多的交换，这是操作系统为了保护系统本身的运行安全，而将内核程序使用的内存空间和用户程序运行的内存空间进行隔离造成的。
...
当读取数据的时候，数据从磁盘复制到内核空间，然后又从内核空间复制到用户空间，是一个很耗时的过程，操作系统为了加速I/O的访问，在内核空间使用缓存机制，也就是将磁盘读取到的文件按一定的组织方式进行缓存，如果用户访问的是同一段磁盘地址的空间数据，那么操作系统将从内核缓冲区直接取出返回给用户程序，也可以降低IO的响应时间
...
同步访问方式就是数据的写入和读取都是同步操作的，与标准的访问文件方式不同的是，只有当数据被成功写入磁盘时才返回给应用程序成功的标志，这种方式性能较差

**异步访问文件的方式**

异步方式就是当访问数据的线程发出请求之后，线程会接着去处理其他事情，而不是阻塞等待，当请求成功后会被通知进行后续的处理。
...
DirectIO和BufferIO相对应。DirectIO就是直接把IO请求提交给底层IO设备，不经过缓存处理。
...
缓冲流和非缓冲流从磁盘的读取数据的调用都是通过底层的read函数来实现，本质上并没有什么区别，但是缓冲流类内部有一个数组byte[] buf,默认长度是8192,对于应用层，调用缓冲类的read的时候，最多可从磁盘上读取8192个字节，后续读取数据都是从数组buf上读取，直到数据全部读取完成。

相对于非缓冲流，缓冲流的IO次数更少，比如每次应用层读取100个字节进行处理，缓冲流一次性读取8192个字节，只需要一次IO,但是非缓冲流每次读取100个字节就需要进行一次IO，效率会低很多，当然非缓冲流也可以一次性读取8192个字节，如此两者基本上就没有什么区别了。
...
**序列化ID的作用：**其实，这个序列化ID起着关键的作用，它决定着是否能够成功反序列化！简单来说，java的序列化机制是通过在运行时判断类的serialVersionUID来验证版本一致性的。在进行反序列化时，JVM会把传来的字节流中的serialVersionUID与本地实体类中的serialVersionUID进行比较，如果相同则认为是一致的，便可以进行反序列化，否则就会报序列化版本不一致的异常。

**序列化ID如何产生：**当我们一个实体类中没有显示的定义一个名为“serialVersionUID”、类型为long的变量时，Java序列化机制会根据编译时的class自动生成一个serialVersionUID作为序列化版本比较，这种情况下，只有同一次编译生成的class才会生成相同的serialVersionUID。譬如，当我们编写一个类时，随着时间的推移，我们因为需求改动，需要在本地类中添加其他的字段，这个时候再反序列化时便会出现serialVersionUID不一致，导致反序列化失败。那么如何解决呢？便是在本地类中添加一个“serialVersionUID”变量，值保持不变，便可以进行序列化和反序列化。
...
适配器模式（Adapter Pattern）是作为两个不兼容的接口之间的桥梁。这种类型的设计模式属于结构型模式，它结合了两个独立接口的功能。就是把一个类的接口变换成客户端所能接受的另一种接口，从而使两个接口不匹配而无法在一起工作的两个类能够在一起工作。

以InputStreamReader和OutputStreamWriter 类为例介绍适配器模式。 
...
Java 中的InputStream和ＯutStream的部分相关子类就使用了装饰器模式。
...
```
FilterInputStream 和FileInputStream都是InputStream的实现类，都实现了InputStream接口的方法，而FileInputStream在FileInputStream的基础上增加了过滤的功能。
...
适配器侧重与不同接口之间的适配，而装饰器是同一个接口下对某个子类功能的增强。
...
##### 8.0.10.3.1. AsynchronousFileChannel 类的使用
...
##### 8.0.10.3.2. synchronousServerSocketChannel 和 AsynchronousSockeChannel类的使用
<a href="#menu"  >目录</a>
...
实际上，函数不可能完全没有副作用。函数会在某个时候返回一个值，而这个值可能是变化的 。 这就是一个副作用。它可能会造成一个内存耗尽的错误，或者是堆枝溢出的错误，导致应用程序崩惯，这在某种意义上就是一个可观测到的副作用 。并且它还会造成写内存、寄存器变化、加载线程、上下文切换和其他确实会影响外界观测的这类事情。
...
* 它绝对不会抛出任何种类的 Exceptio口。它可能抛出错误，例如 OOME( out-of-memory error ， 即内存耗尽）或是 SOE (sta ck-o v erflow eηor ， 即堆栈溢出），但是这些错误表示代码有 bug，并不是作为程序员的你或是你 API的用户应该处理的（除了让应用程序崩溃井最终修复 bug ）。
...
函数式编程和语言无关，并不是哪种语言是函数式编程语言，而是哪种语言对函数式编程更加友好。
...
* 函数式程序更加模块化，因为它们是由只有输入和输出的函数构建的。我们不必处理副作用，不必捕获异常，不必处理上下文变化 ， 不必共享变化的状态，
也没有并发的修改。
* 函数式编程让复合和重新复合更加简单。为了编写函数式程序，你需要开始编写各种必要的基础函数，并把它们复合为更高级别的函数，重复这个过程直到你拥有了一个与你打算构建的程序一致的函数。因为所有的函数都是引
用透明的，它们无须修改便可以为其他程序所重用。
...
相比于普通的循环处理，Ｓtream的代码更加简洁。

性能方面：普通循环对array的操作 对于 堆内存 和 cpu的占用 都是很轻量级的，如果速度快和低内存占用是你的首选，那么建议不要用stream。

调试有点困难：调试器正在进一步开发，但是即使是现在，当你debug步入stream中的代码时，它几乎不能debug到对应地方，但是普通的循环可以很简单的定位到，因为一个普通循环中变量和代码位置联系比较紧密，调试器可以很容易定位到。


**Stream 特性**
* Stream 只负责对数据进行处理、加工，本身不负责存储数据，存储数据是用集合，数组等数据结构。
* Stream 是不可变，一旦修改，就会产生新的Stream对象。Stream不会修改数据源的数据。
* Stream 的操作是一个延迟操作。所有的操作都必须延迟到终结操作时，一起处理(不到最后一步不处理）。
...
中间操作:每次转换原有 Stream 对象不改变，返回一个新的 Stream 对象（可以有多次转换）,这就允许对其操作可以像链条一样排列，变成一个管道。
终止操作(终端操作):每个流只能进行一次终端操作，终端操作结束后流无法再次使用。
...
```java
 ...
 stream2.forEach(e -> System.out.print(e + "\t")); // 0	3	6	9
...
### 12.1.5. Fields和Method定义
<a href="#menu"  >目录</a>

### 12.1.6. 类属性描述
<a href="#menu"  >目录</a>

### 12.1.7. Javap生成的class文件结构
<a href="#menu"  >目录</a>
...
//必须以MBean结尾
//有setter方法才能进行修改
public interface HelloInfoMBean {
...
//类名必须是去掉MBean之后的
public class HelloInfo implements HelloInfoMBean {
...
//格式为xxx:name=xxxx
ObjectName name = new ObjectName("HelloInfoMBean:name=helloInfo");
HelloInfo helloInfo = new HelloInfo();
server.registerMBean(helloInfo,name);
...
类内非构造器获取实例的方法，相比构造器，它有名称，更能标明创建的是什么类型的实例。同时，可以使用预先创建好的实例，从而避免创建不必要的重复对象。第三，可以返回子类型。第四，可以根据参数返回不同的类型。第五，返回的类型在编写该静态工厂方法时可以不存在，而是由实现类决定。



**使用构建器**

使用构建器可以根据自己的需要初始化相关的属性，避免了当类属性较多时使用构造器的麻烦，比如有十几个属性，如果全部初始化，使用构造器，那么构造器参数将会非常多，以及为了满足不同的对象，也需要创建很多构造器。虽然可以通过setter设置，但是相比使用构建器更加优雅。
...
public class Menu{
    ...
    public static class  Builder{
        ...
        public Bulilder name(String name){
            this.name = name ;
            return this;
        }
...
//使用
Menu menu = new Menu.Builder().name("xxxx").build()

```

**用私有构造器或者枚举类强化单例**

如果是单例应该构造器应当为私有的，获取对象只能通过getInstance()。

当然也可以通过枚举类型来创建单例。枚举类型天生单例。
```java
public enum User{
    INSTANCE; 
...
如果能够重用对象，尽量使用单例，避免创建过多的对象造成·频繁的垃圾回收。
...
**其他方法优先于Java序列化**
**谨慎地实现Searializable接口**
**考虑使用自定义的序列化形式**
**保护性编写readObject方法**
**对于实例控制，枚举类型优先于readObject**
**考虑使用序列化代理代替序列化实现**
...
**模式化过程：**把问题抽象化，在忽略掉不重要的细节后，发现问题的一般性本质，并找到普遍适用的过程。
...
职责的耦合有时很难被发现，因为我们习惯于将多个职责一起来考虑
...
```
上面的接口将连接管理和数据收发两个职责放在一起。这两个职责应该被分离吗？答案基本上是肯定的。这两组方法基本上没有任何交集，它们都可以依据不同的原因而变化。进一步说，它们将在应用程序中完全不同的位置被调用，而那些不同的位置将同样会因不同的原因而变化。

```java

public interface CameraImple
{
}
public interface CameraConnect implements CameraImple
{
    void Connect(string host);
    void Disconnect();
}
public interface CameraDataChannel implements CameraImple
{
    void Send(byte[] data);
    byte[] Receive();
}
...
从上面看，两个接口还是通过CameraImple耦合在一起，好处是相对于其他的应用程序，已经可以通过CameraImple进行一定程度的接口隔离。
...
* 使用基类对象指针或引用的函数必须能够在不了解衍生类的条件下使用衍生类的对象。通俗来讲：父类出现的地方子类也可以出现。方便实现类替换，减少代码修改。

通俗理解就是：子类可以扩展父类的功能，但不能改变父类原有的功能。有以下几个引申含义：
* 子类可以实现父类的抽象方法，但不能覆盖（重写）父类的非抽象方法。
* 子类中可以增加自己特有的方法。
* 当子类的方法重载父类的方法时，方法的前置条件（方法的输入，入参）要比父类的入参更宽松（相等也会有问题）。
* 当子类的方法实现父类的方法时（重写，重载，实现抽象方法），方法的后置条件（输出、返回值）要比父类更严格或相等。否则替换时会出现异常。
...
//子类比父类方法前置条件更宽松
//当父类为func1(HashMap map)，子类为func1(Map map)
//输出
Father func1
Father func1
//子类比父类方法前置条件一样或者更严格
//一样：当父类为func1(HashMap map)，子类为func1(HashMap map)
//更紧：当父类为func1(Map map)，子类为func1(HashMap map)
//输出
Father func1
Son func1 
```
从上面可以看出：只有子类比父类方法前置条件更宽松，才会符合里氏替换原则
...
对于更加传统的软件开发方法，例如结构化的分析与设计（Structured Analysis and Design），更趋向于创建高层模块依赖于低层模块的软件结构，进而使得抽象依赖了具体实现细节。而且实际上这些方法最主要的目标就是通过定义子程序的层级关系来描述高层模块式如何调用低层模块的。因此，一个设计良好的面向对象程序的依赖结构是 “inverted” 倒置了相对于传统过程化方法的依赖结构。
...
有些场景中，一部分客户需要接口A的部分功能，另一部分客户需要接口A的另一部分功能，此时Ａ接口拆分成A1和A2接口，使用时根据需求继承相应的接口。

接口隔离的规范约束
* 接口尽量小，不出现臃肿的接口，但是不能无限小
* 接口要高内聚，减少对外的依赖
...
最少知识原则（Least Knowledge Principle），或者称迪米特法则（Law of Demeter），是一种面向对象程序设计的指导原则，它描述了一种保持代码松耦合的策略。其可简单的归纳为：每个单元对其他单元只拥有有限的知识，只了解与当前单元紧密联系的单元；

应用到面向对象的程序设计中时，可描述为 "类应该与其协作类进行交互但无需了解它们的内部结构"。核心观念就是类间解耦，弱耦合。但产生的结果是需要大量的中间类进行交互，因此设计时需要权衡，既降低耦合，又让结构清晰。

如果一个类公开的方法和属性越多，修改时涉及的面也会越多，变更引起的风险也会越大。因此在设计时尽量减少方法和属性的访问权限。
...
![UML类图](pic/java/设计模式/UML类图.png)
表示类的框分成以下几层
* 类名
* 属性清单
* 方法清单
* 性质清单
* 如果一个类有内部成员类，它的类图就有5层。除了类名层不能省略之外。其他层都是可以省略的。
...
* 实现（Realization
* 依赖(Dependency)
    * 依赖关系表示一个类依赖于另一个类的定义。依赖总是单向的。依赖关系中，可以简单的理解，就是一个类A使用到了另一个类B，而这种使用关系是具有偶然性的、临时性的、非常弱的，但是B类的变化会影响到A。一般而言，依赖关系在Java语言中体现为局域变量、方法的形参，或者对静态方法的调用
* 关联（Association)
    ...
    * 自关联
        * 在系统中可能会存在一些类的成员变量为该类本身
    * 多重性关联
        * 多重性关联关系表示两个关联对象在数量上的对应关系。在UML中，对象之间的多重性可以直接在关联直线上用一个数字或一个数字范围表示
* 聚合（Aggregation）
...
![UML多重性说明](pic/java/设计模式/UML多重性说明.png)

比如说唐三藏和徒弟的关联关系，唐只有一个，旁边标1或者1..1。而徒弟是0到多个，旁边标0..*。
...
![UML时序图](pic/java/设计模式/UML时序图.png)
...
```
上面的代码添加了一个方法readResolve，是为了解决序列化反序列化后两个对象不是同一个对象的问题。在进行反序列化时，如果没有检测到readResolve方法，则按照常规的流程通过反射创建一个新的对象，此时就会破坏掉单例模式。如果检测到这个方法，则按照方法中的定义返回，如上代码，如果单例实例已经创建，则返回创建后的实例，否则返回null。

饥饿式单例存在的问题是，如果系统中并没有使用单例，但在类加载时还是创建了，如果存在这种类较多，无疑对内存是一种浪费。饱汉式创建单例可以解决这个问题，只有在需要的时候才会创建实例。
...
public class Singleton2 implements Serializable {
    private final static Singleton2 singleton = null;
    private Singleton2(){
...
public class Singleton3 implements Serializable {
    ...
    private final Singleton3 singleton = null;
    private Singleton3(){
...
这篇文档说存在的问题是，由于指令重排序，导致第一个线程创建实例后，第二个线程读取到的确是null值，导致创建了两个实例
...
进入synchronized块：synchronized 块内使用到的变量从线程的工作内存中清除，这样在 synchronized 块内使用到该变量时就不会从线程的工作内存中获取，而是直接从主内存中获取

退出 synchronized 块的内存语义是把在 synchronized 块内对共享变量的修改刷新到主内存

**volatile关键字**

内存语义：当一个变量被声明为 volatile时，线程在写入变量时不会把值缓存在寄存器或者其他地方，而是会把值刷新回主内存 。当其他线程读取该共享变量时，会从主内存重新获取最新值，而不是使用当前线程的工作内存中的值

因此上面的代码只使用synchronized应该是没有问题的。

**使用静态内部类来创建**

由于内部类是静态的，因此是线程安全的，对象也是在首次调用内部类的时候才会创建(内部类是延迟加载)。
```java
...
以上的四种方式存在的问题是仍然能通过反射来创建不同的实例,但实际开发中，程序员知道应该使用单例的情况下，不应当通过反射来创建不同的实例。
```java
...
```
newInstance()方法中，如果是枚举类型，则会抛出异常
```java
...
使用枚举类来实现单例可以规避上面反射的问题，但是枚举类本质上上是对Enum类的扩展，并且默认是final类，因此不能扩展其他的类以及创建子类，但可以实现接口。
...
public class Factory{
    ...
    public static Api createApi(params...){
        Api api = new ApiImpl();
        //进行各种初始化配置
        return api;
    }
...
public interface Creator{    
    //通过类作为参数
    <T extends Product> T ConcreateProduct(Class<T> c);
    //通过类型标识符作为参数
    <T extends Product> T ConcreateProduct(type);
}
//变种１：回退到简单工厂，使用静态方法
public class Creator{    
    public static Product concreateProduct(){
...
* 工厂方法和简单工厂的方法的区别是将选择具体实现的功能放在工厂子类里实现.
...
public class ProductBuilder implements Builder {
    ...
    public ProductBuilder uid(int uid){
        this.uid = uid;
        return this;
    }
...
Product product = ((ProductBuilder) builder).name("zhangsan").uid(1001).build();

```

对于一个实例对象的初始化，一般方式有通过构造器赋初值，这种方式需要编写较多的构造器来适应不同的使用场景。另一种是通过setter赋初值，可能会出现遗漏。

因此当需要赋初值较多的类，可以使用建造者模式，实例构建过程较为简洁。
...
public class Family implements Cloneable {
    ...
    protected Object clone() throws CloneNotSupportedException {
        ...
        People people = (People)this.people.clone();
...
Family family = new Family(1001,people);
Family family1 = (Family)family.clone();
```
以上就是通过clone来实现原型模式。这里需要注意浅拷贝和深拷贝的问题。要实现深拷贝，除了数值包装类型和String类型变量，其他的引用类型必须也同时实现Cloneable接口。

原型模式本质上调用clone后，先分配内存空间，再将相关数据复制到新的内存空间，然后再让引用变量指向新的内存空间地址。

假如引用类型参数没有实现Cloneable接口，那么新生成的对象和原对象的该引用还是会指向同一个地址，修改其中一个的这个参数时，另一个的值也会同步改变。
...
* 模板方法模式（Template Pattern），又叫模板模式，在一个抽象类公开定义了执行他的方法的模板。它的子类可以按需要重写方法实现，但调用将以抽象类中定义
* 简单说，模板方法模式定义了一个操作中的算法的骨架，而将一些步骤延迟到子类中，使得子类可以不改变一个算法对的结构，就可以重定义该算法的某些特定步骤
* 这种设计模式属于行为型设计模式
...
**注意事项与细节**
* 基本思想是：算法只存在于一个地方，也就是在父类中，容易修改。需要修改算法是，只要修改父类的模板方法或者已经实现的某些步骤，子类就会继承这些修改
* 实现了最大代码复用。父类的模板方法和已经实现的某些步骤会被子类继承而直接使用
* 即统一了算法，也提供了很大对的灵活性。父类的模板方法确保了算法的结构保持不变，同时由子类提供部分步骤的实现
* 该模式的不足之处：每一个不同的实现都需要一个子类实现，导致类的个数增加，使得系统更加庞大
* 一般模板方法都加上final关键字，防止子类重写模板方法
* 模板方法模式使用场景：当要完成在某个过程，该过程要执行一系列步骤，这一系列的步骤基本相同，但其个别步骤咋实现时可能不同，通常考虑用模板方法来处理
...
public class StaticProxy implements UserService {
   ...
   private UserService userService ;
    ...
    public int work() {
        ...
        before();
        int result = this.userService.work();
        after();
        return result;
    }
    private void  before(){
...
proxy.work();
```
静态代理存在的问题是每一个接口都要编写对应的代理类，当被代理的对象过多时，造成程序代码量上升。因此实际中一般使用动态代理。
...
**本质就是多个类之间没有交互，降低耦合，而是通过一个中间类来进行交互。**
