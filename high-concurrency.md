...
- [1. 高并发网站设计](#1-高并发网站设计)
    ...
    - [1.7. 回滚机制](#17-回滚机制)
        - [1.7.1. 事务回滚](#171-事务回滚)
        - [1.7.2. 代码库回滚](#172-代码库回滚)
        - [1.7.3. 部署版本回滚](#173-部署版本回滚)
        - [1.7.4. 静态资源回滚](#174-静态资源回滚)
    - [1.8. 压测与预案](#18-压测与预案)
        - [1.8.1. 系统压测](#181-系统压测)
            - [1.8.1.1. 线下压测](#1811-线下压测)
            - [1.8.1.2. 线上压测](#1812-线上压测)
        - [1.8.2. 系统优化和容灾](#182-系统优化和容灾)
    - [1.9. 缓存](#19-缓存)
        - [1.9.1. 应用级缓存](#191-应用级缓存)
            - [1.9.1.1. 缓存回收策略](#1911-缓存回收策略)
            - [1.9.1.2. 回收算法](#1912-回收算法)
                - [1.9.1.2.1. FIFO](#19121-fifo)
                - [1.9.1.2.2. LRU](#19122-lru)
                - [1.9.1.2.3. LFU](#19123-lfu)
                - [1.9.1.2.4. LRFU](#19124-lrfu)
            - [1.9.1.3. Java 缓存类型](#1913-java-缓存类型)
                - [1.9.1.3.1. 堆缓存](#19131-堆缓存)
                - [1.9.1.3.2. 对外缓存](#19132-对外缓存)
                - [1.9.1.3.3. 磁盘缓存](#19133-磁盘缓存)
                - [1.9.1.3.4. 分布式缓存](#19134-分布式缓存)
                - [1.9.1.3.5. 多级缓存](#19135-多级缓存)
            - [1.9.1.4. 应用级缓存示例](#1914-应用级缓存示例)
            - [1.9.1.5. 缓存使用模式实践](#1915-缓存使用模式实践)
                - [1.9.1.5.1. Cache-Aside](#19151-cache-aside)
                - [1.9.1.5.2. Cache-As-SOR](#19152-cache-as-sor)
                - [1.9.1.5.3. Read-Through](#19153-read-through)
                - [1.9.1.5.4. Write-Through](#19154-write-through)
                - [1.9.1.5.5. Write-Behind](#19155-write-behind)
                - [1.9.1.5.6. Copy-Pattern](#19156-copy-pattern)
            - [1.9.1.6. 缓存一致性处理](#1916-缓存一致性处理)
            - [1.9.1.7. 缓存异常处理](#1917-缓存异常处理)
                - [1.9.1.7.1. 缓存穿透](#19171-缓存穿透)
                - [1.9.1.7.2. 缓存击穿](#19172-缓存击穿)
                - [1.9.1.7.3. 缓存雪崩](#19173-缓存雪崩)
                - [1.9.1.7.4. 解决方案](#19174-解决方案)
                - [1.9.1.7.5. 缓存并发问题](#19175-缓存并发问题)
        - [1.9.2. HTTP缓存](#192-http缓存)
            - [1.9.2.1. 浏览器缓存](#1921-浏览器缓存)
            - [1.9.2.2. CDN缓存](#1922-cdn缓存)
            - [1.9.2.3. NGINX缓存](#1923-nginx缓存)
