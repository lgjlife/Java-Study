...
* 不要过度设计
    * 表现
        * 设计的功能远超实际需求
        * 使用复杂的方案进行设计
    * 防止设计中出现复杂的解决方案,复杂的方案会限制扩展能力,产生大量的长期成本,简单的系统更容易维护和扩展
* 设计时考虑扩展性
    * DID方法:Design 20倍的容量,Implement 3倍的容量;Deploy 1.5倍的容量 
    * 提前考虑如何扩展解决方案
...
* 减少DNS查找
    * 一个请求包括DNS查找IP,TCP连接,发送请求,接收请求.DNS查找会花费较多的时间.但是如果把所有的资源都放在同一个域名下.也有不利的一面.
* 仅可能减少页面对象
    * 页面内对象越多,加载的时间越长
    * 应用方式
        * 减少或者合并对象
* 使用同一品牌的网络设备
    ...
    * 应用方式
        ...
        * 其他网络设备(防火墙,负载均衡器等)要买最好的
* 横向复制
    * 横向扩展,复制服务或者数据库来分散负载.复制数据和功能可以使事务更快地扩展 
    * 适用场景
        * 具有非常高的读写比例的数据库
        * 事务增长大于数据增长的系统
    * 应用方式
...
* 拆分不同的东西
    * 适用场景
        ...
        * 大型的复杂系统,需要特别扩展编程资源
    * 应用方式
        * 拆分不同的的业务,比如用户服务,文件服务.数据库拆分类似
* 拆分相近的东西
    * 非常大数据集合,比如大量的用户数据,可以对其进行数据分片
* 设计横向扩展方案
* 采用经济型系统
* 横向扩展数据中心
* 利用云技术进行设计
...
* 放松时序约束
* 利用CDN
    * 大多数CDN利用DNS(域名服务或者域名服务器),从而替站点来提供内容
    * CDN有助于分流高峰期的流量,通常是扩展站点部分流量的经济型方法
    * 总体来说,CDN可以简单快速地分散流量高峰和流量增长
* 使用过期头
* 缓存AJAX调用
* 利用页面缓存
* 利用应用缓存
