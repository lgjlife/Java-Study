...
- [1. 分布式系统](#1-分布式系统)
    ...
    - [1.2. 大型网站的特点](#12-大型网站的特点)
        - [1.2.1. 特点](#121-特点)
        - [1.2.2. 模式](#122-模式)
        - [1.2.3. 大型网站架构要素](#123-大型网站架构要素)
            - [1.2.3.1. 性能](#1231-性能)
            - [1.2.3.2. 可用性](#1232-可用性)
            - [1.2.3.3. 伸缩性](#1233-伸缩性)
            - [1.2.3.4. 扩展性](#1234-扩展性)
            - [1.2.3.5. 安全性](#1235-安全性)
        - [1.2.4. 微服务化](#124-微服务化)
            - [1.2.4.1. 微服务和SOA](#1241-微服务和soa)
            - [1.2.4.2. 微服务架构与传统单体架构的对比](#1242-微服务架构与传统单体架构的对比)
            - [1.2.4.3. 拆分原则](#1243-拆分原则)
            - [1.2.4.4. 服务化管理和治理框架的技术选型](#1244-服务化管理和治理框架的技术选型)
                - [1.2.4.4.1. RPC](#12441-rpc)
                - [1.2.4.4.2. 服务化](#12442-服务化)
                - [1.2.4.4.3. 微服务](#12443-微服务)
            - [1.2.4.5. 服务拆分](#1245-服务拆分)
        - [1.2.5. 架构演进](#125-架构演进)
            - [1.2.5.1. 单机架构](#1251-单机架构)
            - [1.2.5.2. 应用服务器与数据库分开部署](#1252-应用服务器与数据库分开部署)
            - [1.2.5.3. 引入本地缓存和分布式缓存](#1253-引入本地缓存和分布式缓存)
            - [1.2.5.4. 使用应用服务器集群改善网站的并发处理能力](#1254-使用应用服务器集群改善网站的并发处理能力)
            - [1.2.5.5. 引入反向代理实现负载均衡](#1255-引入反向代理实现负载均衡)
            - [1.2.5.6. 数据库读写分离](#1256-数据库读写分离)
            - [1.2.5.7. 数据库按业务分库](#1257-数据库按业务分库)
            - [1.2.5.8. 把大表拆分为小表](#1258-把大表拆分为小表)
            - [1.2.5.9. 使用LVS或F5来使多个Nginx负载均衡](#1259-使用lvs或f5来使多个nginx负载均衡)
            - [1.2.5.10. 通过DNS轮询实现机房间的负载均衡](#12510-通过dns轮询实现机房间的负载均衡)
            - [1.2.5.11. 引入NoSQL数据库和搜索引擎等技术](#12511-引入nosql数据库和搜索引擎等技术)
            - [1.2.5.12. 大应用拆分为小应用](#12512-大应用拆分为小应用)
            - [1.2.5.13. 复用的功能抽离成微服务](#12513-复用的功能抽离成微服务)
            - [1.2.5.14. 引入企业服务总线ESB屏蔽服务接口的访问差异](#12514-引入企业服务总线esb屏蔽服务接口的访问差异)
            - [1.2.5.15. 引入容器化技术实现运行环境隔离与动态服务管理](#12515-引入容器化技术实现运行环境隔离与动态服务管理)
            - [1.2.5.16. 以云平台承载系统](#12516-以云平台承载系统)
            - [1.2.5.17. 架构设计总结](#12517-架构设计总结)
    ...
    - [1.7. 序列化机制](#17-序列化机制)
        ...
        - [1.7.3. 常用序列化方式实现](#173-常用序列化方式实现)
            ...
            - [1.7.3.2. JSON序列化框架](#1732-json序列化框架)
                - [1.7.3.2.1. Jackson](#17321-jackson)
                - [1.7.3.2.2. FastJSON](#17322-fastjson)
                - [1.7.3.2.3. GSON](#17323-gson)
                - [1.7.3.2.4. FastJSON、Gson和Jackson性能对比](#17324-fastjsongson和jackson性能对比)
            - [1.7.3.3. Hessian](#1733-hessian)
            - [1.7.3.4. Protostuff](#1734-protostuff)
            - [1.7.3.5. 序列化框架的选型](#1735-序列化框架的选型)
    - [1.8. 定时任务](#18-定时任务)
    ...
    - [1.13. 流量限流](#113-流量限流)
    - [1.14. 幂等设计](#114-幂等设计)
    - [1.15. 分布式事务](#115-分布式事务)
        - [1.15.1. 基础概念](#1151-基础概念)
            - [1.15.1.1. 什么是事务](#11511-什么是事务)
            - [1.15.1.2. 本地事务](#11512-本地事务)
            - [1.15.1.3. 分布式事务](#11513-分布式事务)
            - [1.15.1.4. 分布式事务产生的场景](#11514-分布式事务产生的场景)
        - [1.15.2. 分布式事务基础理论](#1152-分布式事务基础理论)
            - [1.15.2.1. CAP理论](#11521-cap理论)
            - [1.15.2.2. BASE理论](#11522-base理论)
        - [1.15.3. 分布式事务实现:两阶段提交2PC](#1153-分布式事务实现两阶段提交2pc)
        - [1.15.4. 分布式事务实现:三阶段提交3PC](#1154-分布式事务实现三阶段提交3pc)
        - [1.15.5. 分布式事务实现:TCC方案](#1155-分布式事务实现tcc方案)
        - [1.15.6. 分布式事务实现:利用消息实现最终一致性](#1156-分布式事务实现利用消息实现最终一致性)
        - [1.15.7. 分布式事务实现:最大努力通知](#1157-分布式事务实现最大努力通知)
    - [1.16. 负载均衡算法](#116-负载均衡算法)
    - [1.17. 服务容错设计](#117-服务容错设计)
    - [1.18. 集群](#118-集群)
    - [1.19. 分库分表](#119-分库分表)
    - [1.20. 反向代理&正向代理](#120-反向代理正向代理)
    - [1.21. 客户端优化](#121-客户端优化)
    - [1.22. 服务系统容量评估和性能保障](#122-服务系统容量评估和性能保障)
        - [1.22.1. 架构设计与非功能质量](#1221-架构设计与非功能质量)
        - [1.22.2. 全面的非功能质量需求](#1222-全面的非功能质量需求)
        - [1.22.3. 典型的技术评审提纲](#1223-典型的技术评审提纲)
        - [1.22.4. 性能测试方案的设计和最佳实践](#1224-性能测试方案的设计和最佳实践)
            - [1.22.4.1. 明确压测目标](#12241-明确压测目标)
            - [1.22.4.2. 压测场景设计和压测方案制定](#12242-压测场景设计和压测方案制定)
        - [1.22.5. 压测工具](#1225-压测工具)
            - [1.22.5.1. ab](#12251-ab)
            - [1.22.5.2. jmete](#12252-jmete)
            - [1.22.5.3. mysqlslap](#12253-mysqlslap)
            - [1.22.5.4. sysbench](#12254-sysbench)
            - [1.22.5.5. dd](#12255-dd)
            - [1.22.5.6. LoadRunner](#12256-loadrunner)
            - [1.22.5.7. hprof](#12257-hprof)
    - [1.23. 大数据日志系统的构建](#123-大数据日志系统的构建)
        - [1.23.1. 开源日志框架](#1231-开源日志框架)
            - [1.23.1.1. JDK Logger](#12311-jdk-logger)
            - [1.23.1.2. Apache Commons Logging](#12312-apache-commons-logging)
            - [1.23.1.3. Apache Log4j](#12313-apache-log4j)
            - [1.23.1.4. Slf4j](#12314-slf4j)
            - [1.23.1.5. Logback](#12315-logback)
            - [1.23.1.6. Apache Log4j2](#12316-apache-log4j2)
        - [1.23.2. 日志系统的优化和实践](#1232-日志系统的优化和实践)
            - [1.23.2.1. 开发人员的日志意识](#12321-开发人员的日志意识)
            - [1.23.2.2. 日志级别的设置](#12322-日志级别的设置)
            - [1.23.2.3. 日志的数量和大小](#12323-日志的数量和大小)
    - [1.24. 构建高可用的系统](#124-构建高可用的系统)
        - [1.24.1. 避免系统中出现单点问题](#1241-避免系统中出现单点问题)
            - [1.24.1.1. 负载均衡技术](#12411-负载均衡技术)
            - [1.24.1.2. 热备](#12412-热备)
        - [1.24.2. 提高应用系统自身的可用性](#1242-提高应用系统自身的可用性)
            - [1.24.2.1. 尽可能避免故障](#12421-尽可能避免故障)
            - [1.24.2.2. 及时发现故障](#12422-及时发现故障)
            - [1.24.2.3. 及时处理故障](#12423-及时处理故障)
            - [1.24.2.4. 访问量以及数据量不断上涨的应对策略](#12424-访问量以及数据量不断上涨的应对策略)
    - [1.25. 构建可伸缩的系统](#125-构建可伸缩的系统)
        - [1.25.1. 垂直伸缩](#1251-垂直伸缩)
            - [1.25.1.1. 支撑高访问量](#12511-支撑高访问量)
            - [1.25.1.2. 支撑大数据量](#12512-支撑大数据量)
            - [1.25.1.3. 提升计算能力](#12513-提升计算能力)
        - [1.25.2. 水平伸缩](#1252-水平伸缩)
            - [1.25.2.1. 支撑高访问量](#12521-支撑高访问量)
            - [1.25.2.2. 支撑大数据量](#12522-支撑大数据量)
            - [1.25.2.3. 提升计算能力](#12523-提升计算能力)
...
##### 1.7.3.2.4. FastJSON、Gson和Jackson性能对比
...
### 1.15.3. 分布式事务实现:两阶段提交2PC
<a href="#menu" style="float:right">目录</a>

### 1.15.4. 分布式事务实现:三阶段提交3PC
<a href="#menu" style="float:right">目录</a>

### 1.15.5. 分布式事务实现:TCC方案
<a href="#menu" style="float:right">目录</a>

### 1.15.6. 分布式事务实现:利用消息实现最终一致性
<a href="#menu" style="float:right">目录</a>

### 1.15.7. 分布式事务实现:最大努力通知
<a href="#menu" style="float:right">目录</a>
...
#### 1.23.1.2. Apache Commons Logging

#### 1.23.1.3. Apache Log4j
